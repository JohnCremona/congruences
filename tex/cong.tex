...
resolution for the case~$p=7$ using modular curves; this is the most relevant case as explained in~\S\ref{S:motivation}.
For~(ii), in
...
us to the work of Halberstadt~\cite{Halberstadt-11nonsplit}. We also thank the anonnymous referee for the careful reading of the paper and various helpful comments.
...
\begin{proof} 1) Let $E'$ be a $p$-congruent twist of~$E$. If $E'$ is
  ...
  (not defined over~$K$, since the image is not in a Cartan by condition~{\bf (S)}) by
  $\Q(\sqrt{-1})$ or $\Q(\sqrt{-3})$, respectively; moreover, since $p$ is
  not ramified in the CM field, the image is inside the normalizer of a Cartan and is projectively
