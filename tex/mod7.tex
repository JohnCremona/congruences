...
For $a, b \in K$, we write $E_{a,b}$ for the elliptic curve defined over~$K$ by the short
Weierstrass equation $Y^2=X^3+aX+b$. 

There is a particular type of quadratic twists 
that have properties which are different from the general case, and will need to be treated
separately in several of the results which follow. 
It is convenience for us to give it a name.
We call the $-1$-twist for curves with
$j$-invariant~$1728$ over fields in which $-1$ is not a square the
\emph{special quadratic twist}. In this case $L=K(\sqrt{-1})$ and the
curve has a model of the form $E_{a,0} : y^2 = x^3 + ax$. 
...
normaliser of a Cartan subgroup of $\GL_2(\F_p)$ but not in thev Cartan
itself, and the projective image has order greater than~$2$,
...
over~$\Qbar$, $t$ preserves the Weil pairing.
...
See Example~\ref{Ex:3twists} below for an illustration of the last part of 
...
such that $E_1[p]\cong E_2[p]$.  Then, for all
