...
The description of our method also includes a discussion on how to determine whether $E[p]$ and~$E'[p]$ are isomorphic (ignoring the symplectic structure) in both the irreducible and reducible cases. Moreover, we give two global symplectic criteria for particular types of congruences: Theorem~\ref{T:Cartan} covers congruences between an elliptic curve~$E$ with mod~$p$ image in the normalizer of a Cartan subgroup and a certain quadratic twist of~$E$ and Theorem~\ref{T:j=0} covers a mod~$7$ congruence between 
...
when explicit equations for the latter are known, which is the case for $p=3,5$ and~$11$. Since the method works independently of the image of~$\rhobar_{E,p}$, in particular, it also covers (i) and (ii).
...
in both the reducible and irreducible case; nevertheless, a similar method to the one we describe below, using instead the
