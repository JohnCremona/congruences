...
$L=K(\sqrt{d})$ with $d\in K$ non-square, we denote by~$E^d$ the
unique $K$-isomorphism class of such curves.
...
\begin{lemma}\label{L:quadratic-twist}
Let $E_{a,b}$ be the elliptic curve defined over $K$ by the short
Weierstrass equation $Y^2=X^3+aX+b$, and let $L=K(\sqrt{d})$ be a
quadratic extension (so $d\in K$ is not a square).  Then the quadratic
...
non-square this imples $d=-u^2$ and $-1$ non-square, so
...
\geq 5$) and $\PP \rhobar_{E,p}(G_F) \simeq C_n$. Thus, for each
such~$p$, in the notation of Proposition~\ref{P:twistNumber}, we have
$M=F$ and $M$ is unique, hence~$d$  is unique modulo squares.
\end{proof}
...
not a square in~$K$ we have $E_a^{-1}\cong E_{-4a}$.

Each $E_a$ has CM by $\Q(\sqrt{-1})$. The curves $E_a$ and $E_{-4a}$
are $2$-isogenous (see \cite[p. 336]{SilvermanI}), and hence
$E_a[p]\cong E_{-4a}[p]$ for all~$p>2$. If $-1$ is a square in $K$,
then $-4=(1+\sqrt{-1})^4$ is a 4th power and $E_a$ and $E_{-4a}$ are
isomorphic; moreover, in this case, we have
$\End_K(E_a)\cong\Z[\sqrt{-1}]$ and the mentioned isogeny is the
endomorphism~$1+\sqrt{-1}$.  Otherwise, when $-1$ is not a square
in~$K$, then $E_a$ and~$E_{-4a}$ are quadratic twists by~$-1$.

Our result for these curves is that there are no isomorphisms between
their $p$-torsion, for any~$p>2$, except for $E_a[p]\cong E_{-4a}[p]$,
provided that the image of $\rhobar_{E,p}$ is not too small.
...
Conversely, if $E_{a_1}[p]\cong E_{a_2}[p]$ and the image of
$\rhobar_{E_{a_1},p}$ has order greater than~$8$, then $a_1/a_2
\in\{1,-4\}$ modulo~$(K^*)^4$.
%% \textbf{We need an assumption on the mod~$p$ image being
%%   not too small here}
\end{theorem}
...
holds.  Since $\rhobar_{E_{a_1},p}(G_K)$ has order greater than~$8$,
we have that $\rhobar_{E_{a_1},p}(G_K)\not\cong C_2\times C_2$, and
also that for all quadratic extensions $L/K$ we also have
$\rhobar_{E_{a_1},p}(G_L)\not\cong C_2\times C_2$.
...
extension $K(\sqrt{d})/K$.  By hypothesis the image is not~$C_2\times
C_2$, so this is true for a unique pair $(N,C)$.  Hence the quadratic
extension cut out by $N/C$ must also be the CM extension
$K(\sqrt{-1})$.  Note that we cannot have $\sqrt{-1}\in K$, since then
the image would be contained in a Cartan and it would not be the case
that half the traces were~$0$ (see the proof of
Proposition~\ref{P:twist}).  Hence $L = K(\sqrt{d})=K(\sqrt{-1})$,
proving the claim.
...
are isomorphic over~$K$.  So if $a_1/a_2$ is a square, then it must in
...
E_{a_2}[p]$ as $G_L$-modules, and the image $\rhobar_{E_{a_1},p}(G_L)$
has order geater than~$4$, by part (i) over~$L$ we conclude that the
curves are in fact isomorphic over~$L$.  Since $L$ is a quadratic
extension of~$K$, the claim gives $L=K(\sqrt{-1})$.  Hence
