...
{\bf We have implemented in {\Magma} and {\Sage} the methods from this paper}
...
% Note that (i) is still conjecturally in the non-split cases when $p \neq 13$
%For $p > 11$, the curves satisfying (i) are expected to be CM 
%and that is known to be the case for $p=13$ (see~\cite{BDMTV}) and also for $p \geq 11$ in the case of split-Cartan subgroups~\cite{BPR2013}; 
...
{\bf We first outline the steps briefly, and then consider each in more detail.}
\begin{enumerate}[1.]
\item Partition the set of isogeny classes of elliptic curves in LMFDB into subsets~$S$ such that curves whose mod~$p$ representations have  isomorphic semisimplifications belong to isogeny classes in the same subset~$S$; {\bf discard all subsets of size~$1$.}
\item For each subset~$S$ prove that the curves in~$S$ really
...
\item For each irreducible subset~$S$, and each pair of isogeny
  ...
  antisymplectic; then use the {\bf isogeny criterion} to partition the set
...
\item For each reducible subset~$S$, determine whether, for each pair
  ...
  $p$-isogenous to it.  If so, proceed as in the previous step.
\end{enumerate}
We now explain each step in further detail.  For the first three
steps, $p$ is arbitrary {\bf while for the last two we restrict to $p=7$}.
...
we use a criterion of Kraus--Osterl\'e (\cite[Proposition~4]{KO}) based on the Sturm bound to
either prove isomorphism up to semisimplification, or reveal a ``false
positive''.  The latter happens when two curves which are not  mod~$p$ isomorphic have traces of Frobenius~$a_{\ell}$ which are congruent modulo~$p$
for all~$\ell\in\calL_B$.

{\bf Example.} After running Step~1 using $B=30$ for $p=7$, of the $20138$
...
\}
\]
and discard the first one as it became a singleton.
...
{\bf Example.} Of the 20138 nontrivial sets from Step~2 we find that 19883
are irreducible, {\it i.e.} consist of curves whose
mod~$7$ representations are irreducible, while 255 are reducible.

The sizes of these sets are as follows:  the irreducible sets have size at most~$5$ and  most (18334) have size~$2$; the reducible sets
have size up to~$76$.
...
In which sense are twists being considered here? For the moment I don't think it is interesting enough to include tables.
]
...
classes of curves $E$, $E'$ such that 
the $G_{\Q}$-modules $E[p]$ and $E'[p]$ are isomorphic and irreducible. Moreover, from \cite[Corollary~3]{FKSym} we know that all isomorphisms $\phi : E[p] \simeq E'[p]$ have the same symplectic type. We wish to determine whether this type is symplectic or anti-symplectic.  

{\bf For all odd~$p$ we may
...
{\bf identify two such isomorphisms~$\phi$ when one is a scalar multiple of
the other.}
...
We note that it is not necessary to apply both
the symplectic and antisymplectic tests to a pair $(E,E')$ if we know
already that $E[7]\cong E'[7]$, since {\bf (in this current irreducible
...
\[
   ...
   \]
{\bf Hence the roots of the degree~$8$ polynomial~$(t^{2} + 13t + 49) \cdot
...
field~$\Q$, which in turn {\bf uniquely identifies the $*$ component of the
representation.}
