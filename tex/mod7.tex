...
composition
\[
\eps_d \; : G_K \to \rhobar_{E,p}(G_K) \twoheadrightarrow
\rhobar_{E,p}(G_K)/\rhobar_{E,p}(G_M) \simeq \{\pm 1 \}.
\]
...
\item More generally, when $E'$ is the twist of~$E$ by a
  cocycle~$\psi$, there is an isomorphism $\alpha:E\to E'$ defined
  over an extension of $K$ of degree at most~$6$, satisfying
  \begin{equation}\label{E:iso-twist-general}
\alpha(\sigma(P)) = \psi(\sigma)\sigma(\alpha(P)) =
\sigma(\alpha(P))\psi(\sigma)\qquad\text{for all~$\sigma\in G_K$ and
  $P\in E(\Kbar)$};
  \end{equation}
  that is, $\alpha^{\sigma} = \psi(\sigma)\alpha$.
...
\subsection{Congruences between twists}
The main constraint restricting the possibility of congruences between
twists is the following result.

\textbf{Include, either here or somewhere in the introduction, a
  definition of $E[p]$ being ``symplectically rigid'' as $G_K$-module,
  meaning that the only $G_K$-equivariant automorphisms of $E[p]$ are
  scalars.  This includes the case where $E[p]$ is irreducible and (?)
  when the image of $G_K$ contains an element of order~$p$.  The in
  the following theorem and corollary we can replace ``absolutely
  irreducible'' by ``symplectically rigid''.}
...
If $E[p]$ (and hence also $E'[p]$) is absolutely irreducible both as
$G_K$-module and also as $G_L$-module, then the twist is trivial; that
is, $E$ and~$E'$ are isomorphic over~$K$ itself.
\end{theorem}

\begin{proof}
Let $\alpha:E\to E'$ be an isomorphism defined over~$L$ satisfying
$\alpha^\sigma=\psi(\sigma)\alpha$ as in Lemma~\ref{L:twist-rep},
where $\psi$ is a cocycle trivial on~$G_L$.  Let $\alpha_p$ denote the
induced map $E[p]\to E'[p]$, which is $\F_p$-linear and also satisfies
the twist relation $\alpha_p^\sigma=\psi(\sigma)\alpha_p$, so
$\alpha_p$ is $G_L$-equivariant.  We are also given a
$G_K$-equivariant isomorphism $\varphi:E[p]\to E'[p]$.  Since both
$\alpha_p$ and~$\varphi$ are $G_L$-equivariant isomorphisms between
the absolutely irreducible $G_L$-modules $E[p]$ and~$E'[p]$, they
differ by a scalar, so there exists $n\in\F_p^*$ such that
$\alpha_p=[n]\circ\varphi$.  (Here, $[n]$ denotes the multiplication
by~$n$ endomorphism of both $E[p]$ and $E'[p]$.)  But $[n]$ is
$G_K$-equivariant, hence so is $\alpha_p$: that is, $\alpha_p^\sigma =
\alpha_p$ for all~$\sigma\in G_K$.

It remains to show that $\alpha$ itself is $G_K$-equivariant;
equivalently, that $\psi$ is trivial.  For all $\sigma\in G_K$, we
have
\[
\alpha_p = \alpha_p^\sigma = \psi(\sigma)\alpha_p,
\]
so, since $\alpha_p$ is an isomorphism, $\psi(\sigma)-1 = 0$ as an
...
divisible by any prime if $\zeta$ has order greater than~$2$.

Since $\psi$ is trivial, $E\cong E'$ over~$K$ as required.
\end{proof}

We are only interested in congruences between curves~$E$ with
absolutely irreducible mod~$p$ Galois representations.  The preceding
theorem shows that very few twists of~$E$, if any, can be
...
We define the \emph{projective $p$-division field} of an elliptic
curve~$E$ defined over $K$ to be the fixed field of the kernel of the
projective representation~$\PP \rhobar_{E,p}$.  Note that both the
...
Note that under the assumptions of the corollary, when $[L:K]=2$ we
have $K\subsetneq L\subsetneq F$.  This is the case for quadratic
twists, and for the special quartic twist by~$-4$ discussed in
Remark~\ref{R:2-isog}.
...
Let $E/K$ and $E^d/K$ be non-isomorphic quadratic twists. Assume that,
for some odd prime $p$, we have $E[p]\cong E^d[p]$ as $G_K$-modules,
and that the projective image $\PP\rhobar_{E,p}(G_K)$ has order
greater than~$2$.
...
\textbf{I adjusted the statement to take into account the case
  $j=1728$, removing the comment I had added earlier.  In fact the
  comment was wrong!  When the projective image is $C_2\times C_2$ the
  construction does give three distinct and non-trivial twists, they
  are just not all quadratic twists (in the strict sense) when
  $j=1728$.}

\begin{proof}
The assumption that the projective image has order greater than~$2$
implies that $E[p]$ is absolutely irreducible, by Lemma~\ref{L:irred}.
...
\begin{corollary}\label{C:CM-twist}
  ...
  order of (negative) discriminant~$-D$.  Set $F=\Q(\sqrt{-D})$.

  \begin{enumerate}
  \item
    For $D\not=4$: $E[p] \simeq E^{-D}[p]$ for all primes $p \geq 5$
    unramified in~$F$ and of good reduction for~$E$.  This congruence
  ...
  \item
    ...
    $E$ and $E'$ are isomorphic over~$F$ but not over~$\Q$.  Again,
    $E[p] \simeq E'[p]$ for all primes $p \geq 5$ unramified in~$F$
...
otherwise.  In all cases this isogeny becomes an endomorphism after
base-change to~$F$.
\end{proof}
...
nature of congruences between quadratic twists. Since the projective
image is dihedral in such cases (by Proposition~\ref{P:twist}), this
...
\subsection{Twisting congruences}
...
\section{The symplectic type of congruences between twists}
...
\begin{itemize}
 ...
 \item[(iii)] $n=3$ and $d=3$ if $j = 0$ (cubic twists);
 \item[(iv)] $n=6$ and $d \in \{3,6\}$ if $j = 0$ (sextic twists);
\end{itemize}
...
In these basis, the map~$t_p$ is represented by the identity matrix in~$\GL_2(\F_p)$ and we obtain, for all $\sigma \in G_K$, the equality of matrices
...
\end{equation}
The map $\sigma \mapsto \Psi_\sigma$ becomes an homomorphism over an extension $M/K$ given by $M=K$ if $n=2$, $M=K(\sqrt{-1})$ if $n=4$ and $M=K(\sqrt{-3})$ if $n=3,6$.
...
We start with the case of quadratic twists.

\begin{theorem}\label{T:quadratic} Suppose that $E'$ is a non-trivial quadratic twist of~$E$ so that $\Psi_\sigma \in \{\pm \Id \}$. Assume that $E[p] \cong E'[p]$ so that $\rhobar_{E,p}$ has image contained in the normalizer of a Cartan subgroup~$C$ but not in~$C$ by Proposition~?? 

Let~$\delta \in \F_p^*$ be a square if $C$ is split and a non-square if $C$ is non-split.
Then $E[p]$ and~$E'[p]$ are symplectically isomorphic if and only if $-\delta$ is a square mod~$p$ and anti-symplectically otherwise.
\end{theorem}
\begin{proof} 
...
that~$M \in C$ and $M$ is non-scalar since the twist is non-trvial. Moreover, for $\tau \in G_K - H$ we have 
$M \rhobar_{E,p}(\tau) M^{-1} =-\rhobar_{E,p}(\tau)$ and so, modulo scalars, $M$ comutes with 
$\rhobar_{E,p}(\tau)$, so $M$ comutes with the projective image, thus $M$ has order 2 modulo scalars ({\bf why?}). 
We conclude that $\Tr M = 2a = 0$, so $a=0$ (as $p \neq 2$) and $\det M = -b^2\delta$. Thus $\det M$ is a square mod~$p$ if and only 
if~$(-\delta/p) = 1$, as desired.
\end{proof}



\begin{lemma} \label{L:LinF}
Let $E$ and $E'$ be $p$-congruent non-trivial $n$-twists by~$u$. 
Let $L = K(\root n \of u)$. 

Then $L \cap F \neq K$. Furthermore,
$L \subset F$ if $n \in \{2,3\}$ and $K(\sqrt{u}) \subset F$ if $n \in \{4,6\}$.
\end{lemma}
\begin{proof} 
%Let~$\sigma$ be in the kernel of~$\rhobar_{E,p}$. Then equality~\eqref{E:PsiM} implies~$\Psi_\sigma = \Id$. Since~$\Psi_\sigma$ becomes a homomorphism when restricted to~$G_{K(\sqrt{-3})}$ we conclude that the kernel of $\rhobar_{E,p}|_{G_{K(\sqrt{-3})}}$ is contained in the kernel of $\Psi_\sigma|_{G_{K(\sqrt{-3})}}$.



For all $\sigma \in G_L$ we have $\Psi_\sigma = \Id$. From~\eqref{E:PsiM} it follows that $M$ comutes with $\rhobar_{E,p}(G_L)$. Since $M$ is not scalar (as the twist is non-trivial),
we conclude that $\rhobar_{E,p}(G_L)$ is abelian. Since $\PP \rhobar_{E,p}(G_K) \simeq D_m$ implies that 
$\rhobar_{E,p}(G_K)$ is
absolutely irreducible, we conclude that $\PP \rhobar_{E,p}(G_L)$ is not dihedral hence the projective image shrank by base change to~$L$, that is $F \cap L \neq K$.

We divide the last statement into three cases. We already know $F \cap L \neq K$.

(a) Suppose $n=2, 3$. Then $L \subset F$ as~$n$ is prime.

(b) Suppose $n=4$. The subfields of~$L$ are
$K$, $K(\sqrt{u})$ and $L$. 
Thus $K(\sqrt{u}) \subset F \cap L$.

(c) Suppose $n=6$. We have $L = K(\sqrt{u},\root 3 \of u)$.
If $F \cap L = K(\sqrt{u})$ or $L$ we are done.

Suppose $F \cap L = K(\root 3 \of{u})$. 
Let $W$ and $W'$ denote the base change of $E$ and $E'$ to $K(\root 3 \of{u})$, respectively.
Let $F_W$ be the field cut of by $\PP \rhobar_{W,p}$. We have $F = F_W$ as 
$K(\root 3 \of{u}) \subset F$. 

The curves $W$ and $W'$ are $p$-congruent quadratic twists and $\PP \rhobar_{W,p} (G) \cong D_m$ for $m \geq 2$, 
as $K(\sqrt{-3}) \not\subset K(\root 3 \of{u})$. From (a) applied to~$W$ and~$W'$ 
we get~$K(\root 3 \of{u})(\sqrt{u}) \subset F_W = F$.
\end{proof}
...
Let $E'/K$ be a cubic or sextic twist of~$E/K$ by~$u \in K$. Assume $E[p] \cong E'[p]$
