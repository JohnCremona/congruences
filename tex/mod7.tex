...
\begin{enumerate}
\item Assume that we are not in the special case $j(E)=1728$ and
  $K(\sqrt{d})=K(\sqrt{-1})$.  Then there is an isomorphism
  $\alpha:E\to E^d$ defined over $K(\sqrt{d})$ satisfying
...
\end{equation}
\item
  In the special case, isomorphisms $\alpha: E=E_{a,0}\to
  E_{-4a,0}=E^{-1}$ defined over $K(\sqrt{-1})$ exist, but do not
  satisfy~(\ref{E:iso-twist}).  However, (\ref{E:rho-twist}) still
  holds for all \emph{odd} primes~$p$.
\end{enumerate}
\end{lemma}

\begin{proof}
In the general case we may assume that $E=E_{a,b}$ and
...
In the special case, take $E=E_{a,0}$ and $E^{-1}=E_{-4a,0}$.  One
...
special case.  For example, taking $E=E_{1,0}$ and $E^{-1}=E_{-4,0}$
over~$\Q$, the image of $\rhobar_{E,2}$ has order~$2$ while
$\rhobar_{E^{-1},2}$ is trivial.
\end{remark}
...
Since we are not in case~(1), we have by Lemma~\ref{L:twist-rep} an
...
Galois equivariant: in fact by~(\ref{E:iso-twist}) we have
...
Let $\sigma$ be the nontrivial automorphism of $K(\sqrt{d})/K$ and
denote by $[-1]$ the negation automorphism on each elliptic curve.
Since we are not in the special case, by Lemma~\ref{L:twist-rep} we
have for $k=1,2$ isomorphisms $\alpha_k:E_k\to E_k^d$, defined
over~$K(\sqrt{d})$, and satisfying
\[
\alpha_k^{\sigma} = [-1]\circ\alpha_k = \alpha_k\circ[-1],
\]
since $\eps_d(\sigma)=-1$.  Let $\phi:E_1[p]\to E_2[p]$ be a
