...
mod~$p$ congruence between~$E$ and its quadratic twist 
by~$d$. We
start by recalling some basic facts about quadratic and higher twists.
...
$Y^2=X^3+aX+b$; every elliptic curve over~$K$ has such as model.

The twists of $E$ over $K$ are parametrised by $H^1(G_K,\Aut(E))$.  If
...
$j(E)\not\in\{0,1728\}$, $j(E)=1728$ and $j(E)=0$ respectively.  In
the latter two cases we make two seemingly trivial, but important,
observations: first, $G_K$ acts nontrivially on~$\Aut(E)$, unless $-1$
or $-3$ (respectively) are squares in~$K$; secondly, there are two
isomorphisms $\calO=\Z[\zeta_n]\cong\End(E)$ (differing by complex
conjugation in $\calO$), where $n=4$ or~$6$ (respectively), and hence
two actions of $\calO^*\cong\Aut(E)$ on~$E$.
...
Kummer isomorphism $H^1(G_K,\Aut(E))\cong K^*/(K^*)^2$.  Explicitly,
the twist of~$E$ by $d$, which is denoted $E^d$ and is called a
\emph{quadratic twist}.  A model for $E^d$ when $E=E_{a,b}$
is~$E^d=E_{ad^2,bd^3}$, and we have a bijection between the set of
quadratic twists of~$E$ and the nontrivial elements of~$K^*/(K^*)^2$.
...
the quadratic twists of~$E$ are parametrised not by $K^*/(K^*)^2$ but
...
\qquad\text{ for all~$P\in E_1[p]$ and~$\zeta\in\calO^*$.}
\end{equation}
In the second case, in order to obtain a new congruence we must apply
\emph{opposite} twists to the two curves.


\begin{proposition}\label{P:twist-cong}
Let~$p$ be an odd prime.  Let $E_1$ and~$E_2$ be elliptic curves
defined over a number field~$K$ such that thgere exists a
$p$-congruence $\varphi:E_1[p]\to E_2[p]$.
\begin{enumerate}
\item
  Let $d\in K^*$; then we also have a $p$-congruence $E_1^d[p]\cong
  E_2^d[p]$ between the quadratic twists.  Moreover, the symplectic
  type of the isomorphism is preserved.
\item
  Suppose that $j(E_1)=j(E_2)=1728$, with $E_1=E_{a_1,0}$ and
  $E_2=E_{a_2,0}$.  If $\varphi$ satisfies
  (\ref{E:forward-congruence}) then for all $u\in K^*$ we also have a
  $p$-congruence $E_{ua_1,0}\cong E_{ua_2,0}$, while if $\varphi$
  satisfies (\ref{E:backward-congruence}) then for all $u\in K^*$ we
  also have a $p$-congruence $E_{u^{-1}a_1,0}\cong E_{ua_2,0}$.
\item
  Suppose that $j(E_1)=j(E_2)=0$, with $E_1=E_{0,b_1}$ and
  $E_2=E_{0,b_2}$.  If $\varphi$ satisfies
  (\ref{E:forward-congruence}) then for all $u\in K^*$ we also have a
  $p$-congruence $E_{0,ub_1}\cong E_{0,ub_2}$, while if $\varphi$
  satisfies (\ref{E:backward-congruence}) then for all $u\in K^*$ we
  also have a $p$-congruence $E_{0,u^{-1}b_1}\cong E_{0,ub_2}$.
%% , unless
...
and~$\alpha_2\circ\varphi\circ\alpha_1^{-1}$ are equal.
...
\begin{example}
$K=\QQ$, $p=3$, $j=1728$.  Projective image $D_4$.
\end{example}

\begin{example}
$K=\QQ$, $p=5$, $j=1728$.  Projective image $D_4$.
\end{example}
...
Hence the projective image is $C_2\time C_2$.  Since
...
Equating these two expressins gives
...
Case 3.  $u$ has order~$6$ in $K^*/(K^*)^6$.  Let $L=K(\root3\of{u})$.
Over~$L$, the isomorphism $E_b\to E_{bu}$ becomes a quadratic twist,
so we apply Case 1.

If $u\equiv-3$ (modulo squares) in $L$ then also $u\equiv-3$ (modulo
squares) in $K$ since $[L:K]$ is odd.
%% Hence (by part 1) $E_b$ and
%% $E_{bu}$ are $3$-isogenous over~$L$, and $E_{bu}\equiv E_{-27b}$
%% over~$L$.
But now $E_{bu}$ is $p$-congruent to $E_{-27b}$ over~$K$,
since both are $p$-congruent to~$E_b$; by Part 2, since $-27/u$ is a
square in~$K$ it is also a $6$th power, so $E_{bu}\cong E_{-27b}$
over~$K$, and so $E_b$ and $E_{bu}$ are $3$-isogenous over~$K$..

Otherwise, $u\not\equiv-3$ (modulo squares) in $L$, so the projectinve
image of $G_L$ must be $C_2\times C_2$ since over~$L$ we have more
than one quadratic twist (by~$u$ and by~$-3$) which are $p$-congruent to~$E_b$.
