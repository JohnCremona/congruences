...
K^*$ not a square. Assume that $E[p]\cong E'[p]$ (as $G_K$-modules)
for some prime $p\ge3$.
...
Furthermore, if $p \geq 5$ is a prime of good reduction of~$E$, then
there is a unique value of~$d$ modulo squares such that $E[p]\cong
E^d[p]$.
\end{proposition}
...
have on the one hand, since $E'=E^d$,
  ...
  E[p]$,
  \[
  a_\frq(E') \equiv a_\frq(E) \mod{p}.
  \]
  Hence for almost all primes~$\frq$ which are inert
  in~$K(\sqrt{d})/K$ we have
  \[
  a_\frq(E') \equiv a_\frq(E) \equiv0 \mod{p}.
  \]
 ...
 The last part follows from group theory: the dihedral group of order
 $2n$ has a unique cyclic subgroup of order~$n$ for $n>2$.
\end{proof}
...
Let $p > 3$ be a prime. Let $E/K$ be an elliptic curve 
...
\begin{proof} The first conclusion follows directly from Proposition~\ref{P:twistNumber}.
...
\item For one curve~$E$ in each isogeny class we computed the set
  ...
  Note that the set~$M_E$ is unchanged if we replace $E$ by a curve
