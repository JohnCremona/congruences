...
E_a:\quad Y^2 = X^3+aX,
\]
with $a$ uniquely determined modulo~$(K^*)^4$, and all such curves are
quartic twists, where the twist by $t\in K^*$ takes $E_a$ to $E_{ta}$.
Each $E_a$ has CM by $\sqrt{-1}$; if $-1$ is not a square in~$K$ then
$E_a$ and $E_{-4a}$ are not isomorphic but are $2$-isogenous, and
hence $E_a[p]\cong E_{-4a}[p]$ for all $p>2$.  When $-1$ is a square
in~$K$ then $\End_K(E_a)\cong\Z[\sqrt{-1}]$ and this isogeny is the
endomorphism~$1+\sqrt{-1}$.  Note also that when $-1$ is a square,
$-4$ is in fact a $4$th power: $-4=(1+\sqrt{-1})^4$.
...
Suppose that $E_{a_1}[p]\cong E_{a_2}[p]$ but that $E_{a_1}$ and
$E_{a_2}$ are not isomorphic over~$K$.

First consider the case when $E_{a_1}$ and~$E_{a_2}$ become isomorphic
over a quadratic extension $K(\sqrt{d})$ of~$K$.  This is the case when
...
Hence $a_1/a_2$ is not a square in~$K$.  Let $m=\sqrt{a_1/a_2}$ and
set $L=K(m)$, a quadratic extension of~$K$.  Since $E_{a_1}[p]\cong
E_{a_2}[p]$ as $G_L$-modules, and $E_{a_1}$ is isomorphic to~$E_{a_2}$
over~$L(\sqrt{m})$, by the previous paragraph (applied over~$L$) we
see that in fact they are isomorphic over~$L$.  But then they are
quadratic twists over~$K$, so $L=K(\sqrt{-1})$.

It remains to show that $a_1/a_2=-4$ (modulo $4$th powers).  Since
$L=K(\sqrt{-1})$ we have that $a_1/a_2$ is minus a square, say
$a_1/a_2=-4s^2$ with $s\in K$.  Also $a_1/a_2\in(L^*)^4$.  Hence
$s^2=t^4$ with $t\in L$; here we use the fact noted above that $-4$ is
a $4$th power in $K(\sqrt{-1})$.  Hence $\pm s=t^2$, a square in~$L$
but not in~$K$.  But~$L=K(\sqrt{-1})$, so $\pm s$ is a square in~$K$
