...
We say that two curves $E/K$ and $E'/K$ are quadratic twists, when they become isomorphic over a quadratic extension~$L/K$.
This is certainly the case when $E' = dE$ for some $d \in K$ not a square, but it includes more cases when $j(E)=0,1728$.
...
Let $E/K$ and $E'/K$ be non-isomorphic elliptic curves 
which become isomorphic over the quadratic extension
$K(\sqrt{d})$. Assume that, 
...
\begin{proof}
Let $\chi$ be the quadratic character of $G_K$ associated to the extension $K(\sqrt{d})/K$. By hypothesis, the curves are quadratic twists by $K(\sqrt{d})$, so
their associated Galois representations are twists by~$\chi$. In particular, for primes~$\frq$ of~$K$ where both curves
have good reduction (so excluding only finitely many primes),
we have
\[
  a_\frq(E') = \chi(\frq) a_\frq(E),
\]
where $a_\frq(E)$ and $a_\frq(E')$ denotes the trace of Frobenius at $\frq$ of~$E$ and~$E'$, respectively. 
  
Moreover, since $E'[p]\cong
  E[p]$, we have
  \[
...
congruences between quadratic twists occur in particular when the image of $\rhobar_{E,p}$ is the normaliser of a Cartan subgroup. The next result  determines the symplectic type of such congruences. Recall from the introduction this is one of the cases where the local methods from~\cite{FKSym} may not apply, as illustrated by Example~\ref{Ex:LocalFail7} below.
...
which has conductor $ N_E = 7^2 \cdot  2381 \cdot 134177^2$
and so it is not in the LMFDB database.

Let $d = -7 \cdot 134177$ and consider~$E^d$ the quadratic twist of~$E$ by~$d$. Applying Theorem~\ref{T:j=1728} (or the general method from Section~\ref{S:statistics}) yelds that
$E[7] \simeq E^d[7]$ are symplectically isomorphic~$G_\Q$-modules and not anti-symplectically isomorphic. Moreover, one can verify that none of the local criteria in~\cite{FKSym} 
...
$E_a$ and~$E_{-4a}$ are not isomorphic but are $2$-isogenous (see \cite[p. 336]{SilvermanI}), and hence $E_a[p]\cong E_{-4a}[p]$ for 
all~$p>2$. If $-1$ is a square in $K$,  then $-4=(1+\sqrt{-1})^4$ is a 4th power and 
$E_a$ and $E_{-4a}$ are isomorphic; 
moreover, in this case, we have 
$\End_K(E_a)\cong\Z[\sqrt{-1}]$ and the mentioned isogeny is the endomorphism~$1+\sqrt{-1}$.  

Note this also shows it is possible for two curves $E_{a_1}/K$ and~$E_{a_2}/K$ to be quadratic twists and $a_1/a_2$ not being a square in~$K$ since, when $\sqrt{-1} \notin K$, the curves~$E_{-4a}$ and~$E_a$ become isomorphic 
over~$K(\sqrt{-1})$ and $-4$ is not a square in~$K$.
...
Suppose that $E_{a_1}[p]\cong E_{a_2}[p]$ {\bf and the image condition holds.}

We claim that if $E_{a_1}$ and~$E_{a_2}$ are not isomorphic over~$K$ and become isomorphic
over a quadratic extension $L=K(\sqrt{d})$ of~$K$
then $\sqrt{-1} \notin K$ and $L=K(\sqrt{-1})$.
Note this holds for the curves 
$E_{-4a}$ and $E_a$: if $\sqrt{-1} \notin K$ then 
they not isomorphic over~$K$, are quadratic twists by~$K(\sqrt{-1})$, and also $2$-isogenous thus have ismorphic
mod~$p$ representations for all odd~$p$.

We will now prove the claim. Suppose $E_{a_1}$ and~$E_{a_2}$ become isomorphic over~$L=K(\sqrt{d})$.
This is the case when
$a_1/a_2$ is a square, but also in other cases: for example, $-4$ is
not a square over~$\Q$ but equals a $4$th power $(1+\sqrt{-1})^4$
over the quadratic extension~$\Q(\sqrt{-1})$.
...
Hence $L = K(\sqrt{d})=K(\sqrt{-1})$, proving the claim.
...
(i) Assume $a_1/a_2 = d^2$ with $d\in K^*$. 
Thus the curves are isomorphic over~$L = K(\sqrt{d})$. 
If $d$ is not a square in~$K$, then the curves are not isomorphic over~$K$ and the claim implies 
$L = K(\sqrt{d})=K(\sqrt{-1})\not= K$.  
But then $-d\in(K^*)^2$, so $a_1/a_2=(-d)^2\in(K^*)^4$, a contradiction. We conclude that $d$ is a square in~$K$ and 
$E_{a_1}$ and~$E_{a_2}$ are isomorphic over~$K$.

(ii) Assume $a_1/a_2$ is not a square in~$K$.  Let $m=\sqrt{a_1/a_2}$ and set $L=K(m)$. Thus $a_1/a_2 = m^2$ with $m \in L$ and 
$E_{a_1}$ is isomorphic to~$E_{a_2}$ over~$L(\sqrt{m})$.
Since $E_{a_1}[p]\cong
E_{a_2}[p]$ as $G_L$-modules {\bf and the condition on the image still holds for~$G_L$}, by applying (i) over~$L$ we
conclude the curves are 
in fact they are isomorphic over~$L$, which is a quadratic extension of~$K$, so $L=K(\sqrt{-1})$ by the claim.

To finish the proof we need to show that $a_1/a_2 \in \{1,-4 \}$ modulo 4th powers. 
