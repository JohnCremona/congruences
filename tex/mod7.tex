...
Note that if $\rhobar_{E,p}$ is absolutely irreducible then $E[p]$
satisfies condition~{\bf (S)} because it satifies condition~(A) in
Proposition~\ref{P:conditionS}.  Moreover, in the case $K=\Q$,
condition~{\bf (S)}is satisfied by $E[p]$ for all~$p\ge7$,
by~\cite[Corollary~3 and Proposition~2]{FKSym}.
...
\jc{rephrase this now that condition~{\bf (S)} is defined in the
  introduction}

Recall that our final objective is to classify the symplectic type of
congruences between twists. Clearly this only makes sense if the
$p$-torsion modules of the relevant curves do not admit
anti-symplectic automorphisms.


Conversely, the next result shows that, in the presence of a $p$-congruence between twists, condition~{\bf (S)} implies that
