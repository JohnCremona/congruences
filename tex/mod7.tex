...
Moreover, the symplectic type of the isomorphism is preserved.
\end{proposition}
\begin{proof}
If $d\not=-1$ or both $b_1$ and~$b_2$ are non-zero, this is
\cite[Lemma 11]{FKSym}.

%% In all cases except for the exceptional case
%% where Let $Y^2=X^3+a_kX+b_k$ for $k=1,2$ be short Weierstrass
%% equations for $E_1$ and $E_2$ respectively.  Let $\sigma$ be the
%% nontrivial automorphism of $K(\sqrt{d})/K$ and denote by $[-1]$ the
%% negation automorphism $(x,y)\mapsto(x,-y)$ on each elliptic curve.
...
As for the symplectic type, the isomorphisms $E_k[p]\cong E_k^{-1}[p]$
have the same type for $k=1,2$ since these twists are both also
$2$-isogenies, so each is symplectic if and only if $2$ is a quadratic
residue modulo~$p$.  Hence the composite $E_1^{-1}[p]\cong E_1[p]\cong
E_2[p]\cong E_2^{-1}[p]$ has the same type as $E_1[p]\cong E_2[p]$.
\end{proof}
