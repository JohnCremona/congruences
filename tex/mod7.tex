...
\end{itemize}
The method we describe in Section~\ref{S:statistics} for $p=7$ could be
...
\jc{For consistency I changed $\Psi_{\sigma}$ to $\Psi(\sigma)$ below,
  and also around (2.3) above.}
...
\jc{you were right, so I also eliminated reference to $E'$ in the
  statement, so I also had to define $E'$ in the following Lemma.
...
\jc{I hope that what I have added helps here.}
...
\jc{I thought it was an interesting corollary that the situation we
  are describing can only happen for half the primes (when $n=4,6$).
  I revised the proof.  Note that we only get two twists, which
  are isogenous to each other, when $n=4,6$ not $n=3$ so there is no
  congruence condition when $n=3$, as you can see directly in (2)(b)
  and implicitly in (2)(a) since all primes $p>3$ are $\pm1\pmod6$!
}
...
\jc{See my answer to your question below about cutting out some
  examples for why I added the following.  It can go (or be reworded)
  if we do cut some of the examples.}
...
\jc{I moved it here as a new subsection}
...
Emmanuel Halberstadt, \emph{Sur la courbe modulaire {$X_{\text{nd\'ep}}(11)$}},
  Ex[.|Math.. \textbf{7}:1 (1998), 163--174.
