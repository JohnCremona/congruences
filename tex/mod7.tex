...
\begin{lemma}
Let $n\in\{4,3,6\}$, and let $K$ be a number field not containing the
...
$n=4$ and $K'=K(\sqrt{-3})$ if $n=3,6$, and $[K':K]=2$.
...
$\sigma(\rnub)=\zeta\rnub$.  Hence $v=\rnub/\rnu\in K'$, and
$\overline{u}=uv^n$.  Hence $\rnub=\rnu v\zeta^k$ for some~$k$;
applying~$\tau$ gives
\[
\rnu = \rnub\;\overline{v}\zeta^{-k} = \rnu v\overline{v},
\]
...
fact that~$\Aut(C_n)\cong\{\pm1\}$ for $n\in\{4,3,6\}$.)

Keeping the notations from the previous lemmas, to prove the first
part of the theorem we will construct a matrix~$A\in\GL_2(\F_p)$
...
We end this section with four families of examples.  These include all
...
$\{\pm x_1, \pm x_2\}$ where $x_1=\frac{1+\sqrt{3}}{1-i}\root4\of{u}$
and $x_2=\frac{1-\sqrt{3}}{1+i}\root4\of{u}$.
...
\begin{comment}
 \nf{How was this checked for arbitrary~$a$? Do we need to check for a
   single~$a$ and apply the proposition for simultaneous twisting?
   I've removed it from this version to keep the paper under 30 pages,
   but maybe this is enough reason to keep it?}
 ...
 \nf{If there is no other way of proving these kind of claims then
   let's add the proposition. Maybe rephrasing it giving less
   importance to the linear and anti-linear part of it?}
\end{comment}
...
$K=\Q$, $p=5$, $j=1728$.  The projective image is the normaliser of a
  ...
  by~$u=a^2/5$).  Only the first holds (by a computation similar to
  the previous example, though a little simpler since we are in the
  split case so do not need to extend scalars).  Since $u=5/a^2$ is
  $+5$ times a square the congruence is symplectic.  There is also a
  congruence with the quartic twist by $-4u=-20/a^2$, which is
  antisymplectic.
...
$K=\Q$, $p=5$, $j=0$.  Denote the curve $E_{0,b}:\ Y^2=X^3+b$
  ...
  %% $5$-congruent,
  hence $E_b$ and $E_{4/(5b)}$ are $5$-congruent.
  This congruence is symplectic; composing with the $3$-isogeny we
  also have an anti-symplectic congruence between $E_b$ and
  $E_{-108/(5b)}$.
\end{example}
...
$K=\Q$, $p=7$, $j=0$.  The projective image is the normaliser of a
  ...
  have either $u\equiv -28/b^2$ or $u\equiv -b^2/28$.
  The first works,
  %% Testing with
...
\jc{You are probably right.  I am not sure that it is worth the
  ...
  \nf{I agree it is not worth it. The most we could do is just to say there are only a few over~$\Q$ and as footnote give the counting in the irreducible case -- it is certainly not worth it entering in the reducible setting -- otherwise you can remove any attempt to give an upper bound even over~$\Qbar$ if you want} 
\end{comment}
