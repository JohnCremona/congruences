...
mod~$p$ congruence between~$E$ and its quadratic twist by~$d$. We
start by recalling some facts about quadratic and higher twists.
...
A model for $E^d$ when $E=E_{a,b}$ is~$E^d=E_{ad^2,bd^3}$, and we have
a bijection between the set of quadratic twists of~$E$ and the
nontrivial elements of~$K^*/(K^*)^2$.

\subsubsection{$j=1728$}
Now $\Aut(E)\cong\mu_4$ as Galois modules.  The twists of $E$ are
parametrized by $H^1(G_K,\mu_4)\cong K^*/(K^*)^4$ and are called
\emph{quartic twists}.  The curve~$E$ has a model~$E=E_{a,0}$, and its
twist by~$u\in K^*$ is $E_{au,0}$, which is isomorphic to~$E_{a,0}$ if
and only if $u\in (K^*)^4$.  The cocycle is given by $\psi(\sigma) =
\sigma(u^{1/4})/u^{1/4} \in \mu_4$, where we identify $\mu_4=\calO^*$
with both $\Aut(E)$ and~$\Aut(E')$ by fixing embeddings
$\calO\hookrightarrow\End(E), \End(E')$ and hence isomorphisms
$\calO^*\cong\Aut(E)\cong\Aut(E')$.  Note that it is possible to do
this so that $\zeta\cdot t(P)=t(\zeta\cdot P)$ for $P\in E(\Kbar)$ and
$\zeta\in\calO^*$.  Then we may view the twisting cocycle~$\psi$ as
taking values in~$\calO^*$, and we have $t^\sigma = \psi(\sigma)t =
t\psi(\sigma)$.


[Explicitly, an isomorphism $t:E_{a,0}\to E_{au,0}$ is given by
  $(x,y)\mapsto(v^2x,v^3y)$ for some fixed choice of
  $v=u^{1/4}\in\Kbar$, and an action of $\zeta\in\mu_4$ is given by
  $(x,y)\mapsto(\zeta^2x,\zeta^3y)$ on both $E_{a,0}$ and~$E_{au,0}$;
  this choice has the property stated, so that~$t$ is an isomorphism
  of $\calO$-modules.]
...
\subsubsection{$j=0$}
Now $\Aut(E)\cong\mu_6$ as Galois modules.  The twists of $E$ are
...
and only if $u\in (K^*)^6$.  The cocycle is given by $\psi(\sigma) =
\sigma(u^{1/6})/u^{1/6} \in \mu_6$.  As before, we identify
$\mu_6=\calO^*$ with both $\Aut(E)$ and~$\Aut(E')$ by fixing
embeddings $\calO\hookrightarrow\End(E), \End(E')$ and hence
isomorphisms $\calO^*\cong\Aut(E)\cong\Aut(E')$, in such a way that
again, $\zeta\cdot t(P)=t(\zeta\cdot P)$ for $P\in E(\Kbar)$ and
$\zeta\in\calO^*$.

[Explicitly, an isomorphism $t:E_{0,b}\to E_{0,bu}$ is given by
  $(x,y)\mapsto(v^2x,v^3y)$ for some fixed choice of
  $v=u^{1/6}\in\Kbar$, and an action of $\zeta\in\mu_6$ is given by
  $(x,y)\mapsto(\zeta^2x,\zeta^3y)$ on both $E_{0,b}$ and~$E_{0,bu}$;
  this choice has the property stated, so that~$t$ is an isomorphism
  of $\calO$-modules.]
...
\textbf{I have left in a simplified version of the following Lemma,
  but maybe we do not need it at all given all the preceding material
  about twists.}
...
there is an isomorphism $\alpha:E\to E^d$ defined over $K(\sqrt{d})$
satisfying
...
\begin{proof}
We may assume that $E=E_{a,b}$ and $E^d=E_{ad^2,bd^3}$ and then define
$\alpha((x,y)) = (dx,d\sqrt{d}y)$ which clearly
satisfies~(\ref{E:iso-twist}) and hence also~(\ref{E:rho-twist}).
\end{proof}
...
Hence reducibility of~$E[p]$ together with $E[p]\cong E^d[p]$ implies that the projective image of $\rhobar_{E,p}$ has
order at most~$2$.
\end{proof}
...
case, $E[p]$ has anti-symplectic automorphisms, so the symplectic type
...
isomorphic to $C_2\times C_2$, so that there are three subgroups~$C\le N$
of index~$2$.
...
Cartan subgroup of $\GL_2(\F_p)$ but not in the Cartan subgroup itself.  In
...
when the projective image has order~$4$, in which case there are three such
quadratic twists.
\end{proposition}
...
\begin{remark}\label{R:C2xC2-degenerate}
The hypotheses of the preceding Proposition always hold when
$j(E)=1728$ and $d=-1$ since then $E$ and $E^{-1}$ are isomorphic.  In
this case, the fact that the projective image is contained in the
normaliser of a Cartan subgroup follows from the fact that $E$ has CM.
The last statement of the proposition must be interpreted as follows
in this case: when the projective image has order greater than~$4$
there is no genuine quadratic twist of $E$ (not isomorphic to $E$
itself) congruent to~$E$, and when the projective image has order~$4$
...
\begin{corollary}\label{C:CM-twist}
  ...
  Then $E[p] \simeq E^{-D}[p]$ for all primes $p \geq 5$ unramified in~$F$ and of good reduction for~$E$.

  Moreover, for each such~$p$, $E^{-D}$ is the unique quadratic twist of~$E$ congruent to~$E$. 

 This congruence is symplectic if and only if $\legendre{2}{p}=+1$
...
isogeny of degree~$2$ for $D=4$, of degree $D/4$ for $D=8,12,16$, and~$28$
and of degree $D$ otherwise.  This isogeny becomes an endomorphism after
base-change to~$F$.

Note that when $D=-1$ the curves $E$ and $E^{-D}$ are in fact
isomorphic.
\end{proof}
...
nature of congruences between quadratic twists. Since the projective image is dihedral in such
cases (by Proposition~\ref{P:twist}), this is a situation where the local methods from~\cite{FKSym} may not apply,
...
\item When $\PP\rhobar_{E,p}\cong C_2\times C_2$, there are three
  different\footnote{with the usual interpretation in case
    $j(E)=1728$, as in Remark~\ref{R:C2xC2-degenerate}.} quadratic
  twists $E^{d}$ of~$E$ such that $E[p] \simeq E^{d}[p]$.  If
  $\sqrt{p^*}\in K$ then all three congruences are symplectic.
  Otherwise, one of the quadratic twists is by $K(\sqrt{p^*})$ and is
  symplectic while the other two are anti-symplectic.
\end{enumerate}
\end{theorem}
...
See Example~\ref{Ex:3twists} below for an illustration of the last
...
$\Aut(E_1)=\Aut(E_2)=\calO^*$ has order~$4$ or~$6$, we either have
...
\qquad\text{ for all~$P\in E_1[p]$ and~$\zeta\in\calO^*$.}
\end{equation}
We call these \emph{direct congruences} and \emph{opposite
  congruences} respectively.  With opposite congruences, in order to
...
\item
  ...
  $E_2=E_{a_2,0}$.  If $\varphi$ is a direct congruence (satisfying
  (\ref{E:forward-congruence})), then for all $u\in K^*$ we also have
  a $p$-congruence $E_{ua_1,0}\cong E_{ua_2,0}$, while if $\varphi$ is
  opposite (satisfying (\ref{E:backward-congruence})), then for all
  $u\in K^*$ we also have a $p$-congruence $E_{u^{-1}a_1,0}\cong
  E_{ua_2,0}$.
\item
  Suppose that $j(E_1)=j(E_2)=0$, with $E_1=E_{0,b_1}$ and
  $E_2=E_{0,b_2}$.  If $\varphi$ is a direct congruence (satisfying
  (\ref{E:forward-congruence})), then for all $u\in K^*$ we also have
  a $p$-congruence $E_{0,ub_1}\cong E_{0,ub_2}$, while if $\varphi$ is
  opposite (satisfying (\ref{E:backward-congruence})), then for all
...
$\varphi_d=\alpha_2\circ\varphi\circ\alpha_1^{-1}$.  Then $\varphi_d$ is
certainly a $G_{K(\sqrt{d})}$-equivariant linear isomorphism, also
satisfying
\[
\varphi_d^{\sigma} =
...
\]
since $[-1]\circ\varphi=\varphi\circ[-1]$.  Hence in fact $\varphi_d$ is
$G_K$-equivariant.

(2), (3) When $\varphi$ satisfies (\ref{E:forward-congruence}) the
argument is very similar, defining $\alpha_k:E_{a_k,0}\to E_{ua_k,0}$
for $k=1,2$ to be the quartic or sextic twist by~$u$, satisfying
\[
\alpha_k^{\sigma} = \psi(\sigma)\circ\alpha_k = \alpha_k\circ\psi(\sigma),
\]
where $\psi$ is the twisting cocycle, with values in~$\calO^*$, and we
choose compatible isomorphism $\calO\to\End(E_k)$ so that the second
equality holds.  Now we define
$\varphi_u=\alpha_2\circ\varphi\circ\alpha_1^{-1}$ and check as before
that $\varphi_u$ is $G_K$-equivariant, using
(\ref{E:forward-congruence}).

When $\varphi$ instead satisfies (\ref{E:backward-congruence}), define
$\alpha_2:E_{a_2,0}\to E_{ua_2,0}$ to be the twist by~$u$,
so that
\[
\alpha_2^{\sigma} = \alpha_2\circ\psi(\sigma),
\]
where $\psi$ is the cocycle associated to~$u$, but now
define~$\alpha_1:E_{a_1,0}\to E_{u^{-1}a_1,0}$, so that
\[ \alpha_1^{\sigma} = \alpha_1\circ\psi(\sigma)^{-1}.
\]
Setting $\varphi_u=\alpha_2\circ\varphi\circ\alpha_1^{-1}$, we may
again check that $\varphi_u$ is $G_K$-equivariant, now using
(\ref{E:backward-congruence}).

Finally, in all cases, since the isomorphisms $\alpha_k$ preserve the
Weil pairing, the symplectic types of $\varphi$
...
\textbf{The old version of the previous Proposition allowed one
  further situation, where $j(E_1)=1728$ but $j(E_2)\not=1728$; to
  $E_1$ we apply the pseudo-quadratic twist which is in fact the
  quartic twist by $-4$, while to $E_2$ we apply the genuine quadratic
  twist by~$-1$, and find that the resulting curves are congruent, the
  type being symplectic iff $2$ is a square.}  \textbf{I don't think
  we need to include this.  For $E_1$ we have applied a $2$-isogeny,
  and to $E_2$ we have done something else which is covered by earlier
  results.  If you agree, that means that both the following 2
  examples can be omitted.}
...
\begin{example} \label{Ex:3twists}
  ...
  of conductor $6534=2\cdot3^3\cdot11^2$. 
  The image of the mod~$3$
  Galois representation is the normalizer of the split Cartan, which is isomorphic to $D_4$.
  The three quadratic subfields of the $3$-division fields
...
Conversely, suppose that $E_{a_1}[p]\cong E_{a_2}[p]$ for some $p>2$,
with projective image of order at least~$4$.
\begin{enumerate}
  \item If the projective image of $\rhobar_{E_{a_1},p}$ has order
    greater than~$8$, then either $E_{a_1}$ and~$E_{a_2}$ are
    isomorphic over~$K$, or $-1\notin(K^*)^2$ and $E_{a_2}\cong
    E_{-4a_1}$, which is $2$-isogenous to~$E_{a_1}$.
  \item If the projective image of $\rhobar_{E_{a_1},p}$ has order
    equal to~$8$, then
  \item If the projective image of $\rhobar_{E_{a_1},p}$ has order
    equal to~$4$, then
...
Suppose that $E=E_a$ is $p$-congruent to $E'=E_{au}$, its quartic
twist by~$u$, where $u$ is not a $4$th power.

First suppose that the projective image has order~$4$, isomorphic to
$C_2\times C_2$.  Now we may apply Proposition~\ref{P:twist},
Remark~\ref{R:C2xC2-degenerate} and Theorem~\ref{T:Cartan}: if $p^*$
is not a square in~$K$ then $E'$ must be the quadratic twist of~$E$
by~$p^*$, the $p$-congruence between $E$ and~$E'$ is symplectic, and
the $2$-isogeny from~$E$ is antisymplectic.  Hence this case can only
occur when $\legendre{2}{p}=-1$, that is, $p\equiv\pm3\pmod8$.  In
case $p^*\in K$, we cannot say more.

From now on assume that the project image has order at least~$8$.

Suppose that $u=v^2\in(K^*)^2$, where $\pm v$ are not squares (else
$u$ is a $4$th power), so that $E$ and~$E'$ are quadratic twists.
Since the projective image has order greater than ~$4$ we know by
Proposition~\ref{P:twist} that there are no congruences between $E$
and any quadratic twist other than the trivial one with the
$-1$-twist, which is isomorphic to $E$ itself.  This implies that
$v=\pm1$ (modulo squares), which is a contradiction, so this case is
impossible.

Now suppose that $u$ is not a square.  Extending the base field
to~$K(\sqrt{u})$, we are in the situation of the previous paragraph,
with the projective image of $G_{K(\sqrt{u})}$ half that of $G_K$.
Hence, if the original projective image has order greater than~$8$ it
follows that $E'$ and $E$ are $2$-isogenous, while if the projective
image has order~$8$ then either $p^*$ is not a square in $K$ and
$p\equiv\pm3\pmod8$ and ...
...
\begin{example}
$K=\Q$, $p=3$, $j=1728$.  Projective image $D_4$, non-split.
\end{example}

\begin{example}
$K=\Q$, $p=5$, $j=1728$.  Projective image $D_4$, split.
\end{example}
...
$E_{-27b}$ are $3$-isogenous (see Remark~\ref{R:3-isog}).

Let $p\ge5$.  Let $K$ be a number field not containing $\sqrt{-3}$ or
$\sqrt{p^*}$, and let $b\in K^*$ be such that the projective image
$\rhobar_{E_b,p}(G_K)$ has order greater than~$4$, hence is dihedral
and absolutely irreducible.  Suppose that $E_b[p]\cong E_{bu}[p]$ as
$G_K$-modules, for some $u\in K^*$ not a $6$th power.

Case 1: $u\in(K^*)^3$.  Then $E_b\cong E_{bu}$ over
$K(\root6\of{u})=K(\sqrt{u})$, and $E_{bu}$ is the quadratic twist
by~$u$ of~$E_b$.  If $u\equiv-3$ (modulo squares), then $u\equiv-27$
(modulo $6$th powers), so $E_{bu}\cong E_{-27b}$ which is
$3$-isogenous to~$E_b$.  Otherwise, $u\not\equiv-3$ (modulo squares),
and $E_b$ is $p$-congruent to more than of its quadratic twists.
Hence the projective image is $C_2\times C_2$.  Since
$\sqrt{p^*}\not\in K$, we have $u\equiv p^*$ or $u\equiv-3p^*$ (modulo
squares).  In either case the $-3$-twist is symplectic, forcing
$\legendre{3}{p}=1$, so $p\equiv\pm5\pmod{12}$.  The original
congruence is either with the quadratic twist by $p^*$, which is
symplectic, or by $-3p^*$ and anti-symplectic.

Case 2: $u\in(K^*)^2$ but $u\not\in(K^*)^3$.  We show that this is
impossible.  Let $L=K(\root6\of{u})=K(\root3\of{u})$.  There is an
isomorphism $\alpha:E_b\to E_{bu}$, defined over~$L$, satisfying
$\alpha^\sigma=\psi(\sigma)\alpha$ for $\sigma\in G_K$, for some
cocycle~$\psi$ with trivial restriction to~$L$.  This induces a
$G_L$-equivariant isomorphism $\alpha_p:E_b[p]\to E_{bu}[p]$.  By
hypothesis we also have a $G_K$-equivariant isomorphism
$\varphi:E_b[p]\to E_{bu}[p]$.  However, both $E_b[p]$ and $E_{bu}[p]$
are absolutely irreducible both as $G_K$-modules and as $G_L$-modules,
the latter since $[L:K]=3$.  By Schur's Lemma, $\varphi$
and~$\alpha_p$ differ by a scalar, so there exists $n\in\F_p^*$ such
that $\alpha_p=[n]\circ\varphi$.  But both $[n]$ and $\varphi$ are
$G_K$-equivariant, hence so is $\alpha_p$.  Thus for all $P\in E_b[p]$
and $\sigma\in G_K$ we have $\alpha_p(P)^\sigma =
\alpha_p^\sigma(P^\sigma) = \alpha_p(P^\sigma)$, and also
$\alpha_p^\sigma(P^\sigma) = \psi(\sigma)\alpha_p(P^\sigma)$.
Equating these two expressions gives
$\alpha_p(P^\sigma)\in\ker(\psi(\sigma)-1)$.  Hence $E_{bu}[p]\subseteq
E_{bu}[\psi(\sigma)-1]$, so $p$ divides $\psi(\sigma)-1$ in
$\End(E_{bu})$.  Since $p>2$ and $\psi(\sigma)\in\mu_6$, this is only
possible if $\psi(\sigma)=1$ for all~$\sigma\in G_K$.  Hence $E_b\cong
E_{bu}$ over~$K$, so $u\in(K^*)^6$, contradiction.

Case 2': $u\not\in(K^*)^3$ and $u\equiv-3$ (modulo squares).  Now
$E_b$ is $p$-congruent to both $E_{bu}$ and $E_{-27b}$, so the latter
are $p$-congruent to each other.  By Case 2, since $-27/u$ is a square
it is also a cube, so $E_{bu}\cong E_{-27b}$ and the congruence is
induced by the $3$-isogeny.

Case 3.  $u$ has order~$6$ in $K^*/(K^*)^6$, with $u\not\equiv1,-3$
(modulo squares).  Let $L=K(\root3\of{u})$.  Over~$L$, the isomorphism
$E_b\to E_{bu}$ becomes a quadratic twist, so we apply Case 1.  Since
$u\not\equiv-3$ (modulo squares) in $L$, the projective image of $G_L$
must be $C_2\times C_2$ since over~$L$ we have more than one quadratic
twist (by~$u$ and by~$-3$) which are $p$-congruent to~$E_b$.  Since
$p^*$ is not a square in $K$ by hypothesis, it is also not a square in
$L$, so $u\equiv p*$ or~$-3p*$ modulo~$(L^*)^2$.  Replacing $u$ by
$u(p^*)^3$ if necessary we may assume that $u\equiv p*$ modulo squares,
in $L$ and hence also in $K$.


Either the projective images of $G_K$ and $G_L$ are equal, or the
latter has index~$3$ in the former.  In the first case, since the
projective image of~$G_K$ is $C_2\times C_2$, we have a $p$-congruence
over~$K$ between $E_b$ and its quadratic twist by~$p^*$ which is
$E_{b(p^*)^3}$ and hence also between $E_{bu}$ and $E_{b(p^*)^3}$.  By
Case 2 again, $u/(p^*)^3$ must be a cube as well as a square, so
$E_{bu}$ is also the quadratic twist of~$E_b$ by~$p^*$.  Hence in this
case the original twist by~$u$ is just the quadratic twist by either
$p^*$ or by $-3p^*$.

Otherwise, the projective image of $G_K$ has order~$12$, isomorphic to
the dihedral group~$D_6$, and $K(\root3\of{u})$ is the unique (up to
conjugacy) subgroup of the $p$-division field~$K(E_b[p])$.  This
uniquely determines $u^{\pm1}$ (modulo cubes).  Since $u$ is uniquely
determined modulo squares (up to multiplication by~$-3$) and cubes (up
to replacing $u$ by $u^{-1}$, this almost uniquely identifies the
sextic twist $E_{bu}$.  Note that there cannot exist sextic twists by
both $u$ and $u^{-1}$ by Case 2, unless $u$ is a cube so that these
are the same twists.

\begin{example}
$K=\Q$, $p=5$, $j=0$.  Projective image $D_6$, non-split.  We expect a
  ...
  $5$-congruent, hence $E_b$ and $E_{4/5b}$ are also.
  Proposition~\ref{P:twist-cong}.

  Symplectic type?
\end{example}

\begin{example}
$K=\Q$, $p=7$, $j=0$.  Projective image $D_4$, split.
\end{example}
