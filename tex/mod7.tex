...
when they are not isomorphic over~$K$ but become isomorphic over a
quadratic extension~$L/K$.  Writing $L=K(\sqrt{d})$ with $d\in K^*$
non-square, we denote by~$E^d$ the unique $K$-isomorphism class of
quadratic twists of~$E$; this only depends on the class of~$d$
modulo~$(K^*)^2$.
...
by~$d$ is $E_{ad^2,bd^3}$ unless $L=K(\sqrt{-1})$ and $b=0$, when we
have $E_{a,0}^{-1}=E_{-4a,0}$.
\end{lemma}
\begin{proof}
...
\cong \rhobar_{E,p}\otimes\eps_d$.  Here, we exclude $p$ which are
ramified in $K(\sqrt{d})$.  See also Corollary~\ref{C:CM-twist}.
\end{remark}
...
Let $E$ be an elliptic curve over the number field~$K$, let $p\ge3$
be a prime, and suppose that we have a congruence $E[p]\cong E^d[p]$
...
When the projective image is dihedral and of order greater than~$2$,
\textbf{it is usually the case that} the image is contained in the
normaliser~$N$ of a Cartan subgroup~$C$ of~$\GL_2(\F_p)$.  In the case
where the projective image is~$D_2\cong C_2\times C_2$, there are there
are three different choices for~$C$.  For $n\ge3$ the Cartan subgroup
is unique.
...
\begin{proof}
  ...
  of order~$2$ in~$\PGL_2(\F_p)$ are those with trace zero, and these
  elements are split (having $2$ fixed points on $\PP^1(\F_p)$) if the
  determinant is minus a square, and non-split (having no fixed
  points) otherwise.  Hence when $p\equiv1\pmod4$, elements of
  order~$2$ are split if and only if they lie in $\PSL_2(\F_p)$, while
  for $p\equiv3\pmod4$ the reverse is the case.  The result follows.
\end{proof}
...
(except when the image is very small) when the image is contained in
the normaliser of a Cartan subgroup but not in the Cartan itself.

\begin{proposition}\label{P:twist}
Let $E/K$ and $E^d/K$ be quadratic twists, where $d\in K^*$ is not a
square. Assume that, for some prime $p\ge3$, we have $E[p]\cong
...
(Lemma~\ref{L:irred}) and have the same traces, they are isomorphic.

For the last part we note that for $n=2$, $D_n$ has three cyclic
subgroups of index~$2$, since $D_2\cong C_2\times C_2$, while for
$n\ge3$ there is only one such subgroup.
\end{proof}
...
For $p$ an odd prime, let $p^*=\pm p\equiv1\pmod4$, so that
...
$E[p]\cong E^d[p]$.
\begin{enumerate}
\item If $K(\sqrt{d})=K(\sqrt{-1})$ and~$j(E)=1728$, the congruence is
  symplectic or antisymplectic according as the Legendre symbol
  $(2/p)=+1$ or~$-1$ respectively.
...
\begin{proof}
(1) When case $j(E)=1728$, we know by Remark~\ref{R:2-isog} that
$E^{-1}$ is $2$-isogenous to~$E$, so the result follows by the isogeny
criterion.  Assume from now on that this is not the case.

(2) As usual, write $\eps_d$ for the quadratic character of~$G_K$
fixing~$K(\sqrt{d})$.

Since we are not in case~(1), an isomorphism~$t: E \to E^d$ defined
over $K(\sqrt{d})$ is given on short Weierstrass models by $(x,y)
\mapsto (dx,d\sqrt{d}y)$.  With a direct calculation we see that
\begin{equation} \label{E:eps}
 t(\sigma(P)) = \eps_d(\sigma)\cdot\sigma(t(P)) \quad \text{ for all } \sigma \in G_K
 \text{ and all } P \in E[P].
\end{equation}
Note that this equation does not hold in case~(1).

We also let $t: E[p](\Qbar) \to E^d[p](\Qbar)$ denote the induced map
on $p$-torsion points; this is an $\F_p$-linear isomorphism which is
not Galois equivariant, on account of the character~$\eps_d$ appearing
in~(\ref{E:eps}).  Also, since $t$ is an isomorphism over~$\Qbar$, it
preserves the Weil pairing.
...
anti-symplectic otherwise. The result now follows by a simple case
checking.
...
always---preserving the symplectic type.
...
$j(E_2)\not=1728$, so that neither twist is the special case.
...
$\alpha_k:(x,y)\mapsto (dx,d\sqrt{d}y)$.  Then by~\ref{E:rho-twist} we
have
\[
...
itself; by Proposition~\ref{P:twist} we have $E_1[p]\cong
...
isomorphisms $E_k[p]\cong E_k^{-1}[p]$ have the same type, since these
twists are both also $2$-isogenies, so each is symplectic if and only
if $2$ is a quadratic residue modulo~$p$.  Hence the composite
...
Remark~\ref{R:2-isog}), and hence $E_a[p]\cong E_{-4a}[p]$ for
...
over~$K$ but are quadratic twists by a quadratic
extension~$L=K(\sqrt{d})$, then $L=K(\sqrt{-1})$ (and $\sqrt{-1}
\notin K$).  For suppose that $E_{a_1}$ and~$E_{a_2}$ become
isomorphic over~$K(\sqrt{d})$.  Applying Proposition~\ref{P:twist},
the image of $\rhobar_{E,p}$ is contained in the normaliser~$N$ of a
Cartan subgroup~$C$, but not contained in the Cartan itself, where $C$
cuts out the quadratic extension $K(\sqrt{d})/K$.  By hypothesis the
projective image is not~$C_2\times C_2$, so this is true for a unique
pair $(N,C)$.  Hence the non-trivial quadratic extension cut out by
$C$ must be the CM extension $K(\sqrt{-1})$; in particular, $\sqrt{-1}
\not\in K$.  Hence $K(\sqrt{d})=K(\sqrt{-1})$, proving the claim.
