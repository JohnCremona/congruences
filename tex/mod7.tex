...
$X_E^-(p)$ parametrizes antisymplectic isomorphisms. Note that the pair $(E,\id)$ constituted by $E$ itself and the identiy map
...
p\le17$.  According to Theorem~\ref{T:cong19} below, no congruences
...
{\bf Example.} After carrying out this step for $7\le7\le17$, using
$B=40$, we find: $20138$ nontrivial subsets for $p=7$, $635$ for
$p=11$, $150$ for $p=13$ and~$8$ for $p=17$.
...
two steps are as follows: for $p=7$ the~$20138$ subsets have sizes
between~$2$ and~$76$; for $p=11$, $p=13$ and $p=17$ they all have
size~$2$.

%% For $p \geq 19$ we find no congruences at all between non-isogenous
%% curves (see Theorem~\ref{T:cong19}).
...
{\bf Example (continued).} For $p=7$, of the 20138 nontrivial sets
from Step~2, we find that 19883 are irreducible, {\it i.e.}\@ consist
of curves whose mod~$7$ representations are irreducible, while $255$
are reducible.
...
Of the $19883$ non-trivial sets of isogeny classes with mutually
isomorphic irreducible mod~$7$ representations, we find that in
$12394$ cases all the isomorphisms are symplectic, while in the
remaining $7489$ cases antisymplectic isomorphisms occur.
...
\subsection{twists}
It is easy to see that if there is a mod~$p$ congruence betwee two
elliptic curves~$E_1$ and~$E_2$, then for any $d\in\Q^*$ there will
also be a congruence (of the same symplectic type) between their
twists $E_1^d$ and~$E_2^d$.  Nevertheless it is hard to say precisely
how many congruences there in the database ``up to twist'', for the
simple reason that twisting changes conductor in general, so we may
have a set of (say) three mutually $7$-congruent elliptic curves in
the database, but one or more of their twists are not in the database.

Instead, to have an idea of how many congruences we found up to twist,
we simply report on how many distinct $j$-invariants we found,
...
$p=17$ All $8$ pairs of $17$-congruent pairs are quadratic twists,
their $j$-invariants being $-46585/243$ and
$48412981936758748562855/77853743274432041397$.
...
of contexts in our study.  The following result characterizes this
situation completely in the case of quadratic twists.

\begin{proposition}\label{P:twist}
Let $E/\Q$ and $E'/\Q$ be elliptic curves which are non-isomorphic
quadratic twists, say $E' = E^d$ for some $d \in \Q^*$. Assume that
...
Moreover, there are either 1 or 3 values of~$d$ with this property,
where the latter case occurs when $\rhobar_{E,p}$ has projective image
$\Z/2\Z \times \Z/2\Z$.

In particular, if $p \geq 5$ is a prime of good reduction of~$E$ then
there is only one such value of~$d$.
\end{proposition}
...
%%     of~$\rhobar_{E,p}$ is contained in the normaliser of
%%     Cartan subgroup of $\GL_2(\F_p)$ but not in the subgroup itself.
...
\subsection{The case of image equal to normaliser of Cartan}
We have seen in the preceding subsection that congruences between
quadratic twists occur when the image is contained in the normaliser
of a Cartan subgroup.  Here we show that the type of congruence
(symplectic or not) is easy to determine in this situation.

Let $p$ be an odd prime, and fix $\zeta_p \in \Qbar$ a primitive $p$-th root of
unity.
...
\end{corollary}
\begin{proof}
One can show that $E_b$ and $E_{-27b}$ are antisymplectically
isomorphic using the same methods as used for Theorem~\ref{T:j=0};
then Theorem~\ref{T:j=0} implies that $E_b$ and $E_{27\cdot28/b}$ are
antisymplectically isomorphic.  However, the corollary is easier to
prove by noting that the curves $E_b$ and~$E_{-27b}$ are
...
\begin{theorem}
 \label{T:cong19}
 Let $p \geq 19$ be a prime. Let $E/\Q$ and~$E'/\Q$ 
 be elliptic curves in the LMFDB database. 
 Suppose that $E[p] \simeq E'[p]$ as $G_\Q$-modules. Then $E$ and $E'$ are $\Q$-isogenous.
 \end{theorem}
\begin{proof} Let $N_E$, $\Delta_E$ and $N_{E'}$, $\Delta_{E'}$ denote the conductor and minumal discriminant of $E$ and~$E'$, respectively. Let $N_p$ and $N'_p$ denote the Serre level (i.e. the Artin conductor away from~$p$)
of~$\rhobar_{E,p}$ and~$\rhobar_{E',p}$ respectively.
We have $N_p = N_p'$ since $E[p] \simeq E'[p]$ as $G_\Q$-modules.

Recall that the conductor of an elliptic curve at primes~$p \geq 5$ divides~$p^2$ and that $N_p \mid \tilde{N}_E$, where $\tilde{N}_E$ denotes~$N_E$ away from~$p$. 
Moreover, from Kraus~??
it follows that, for each $\ell \neq p$, if 
$\vv_{\ell}(N_p) \neq \vv_{\ell}(N_E)$ then
$\vv_{\ell}(N_E) = 1$ and $p \mid \vv_{\ell}(\Delta_E)$.

Therefore, we can find primes $q_0,\cdots,q_n$ such that 
\[
 N_E = p^k \cdot N_p \cdot q_0 \cdot \ldots \cdot q_n, 
 \qquad p \mid \vv_{q_i}(\Delta_E), \qquad 0 \leq k \leq 2
\]
where the number of~$q_i$ occurring is $\geq 1$ if and only if 
$\tilde{N}_E \neq N_p$. The same applies to~$E'$ and, since $N_p = N'_p$, 
we have
\begin{equation}\label{E:condE'}
 N_{E'} = p^{k'} \cdot N_p \cdot q'_0 \cdot \ldots \cdot q'_m, 
 \qquad p \mid \vv_{q'_i}(\Delta_E'),
 \qquad 0 \leq k' \leq 2.
\end{equation}

To prove the theorem, we will run through the database by descending conductors as follows.

Fix a curve $E$ in the LMFDB with the largest conductor. 
...
there are no congruence between any two of them for $p \geq 19$, otherwise $p$ needs to divide the differences. 
Thus we have $N_E > N_{E'}$.
...
Note that we can have $q=p$.
From $E[p] \simeq E'[p]$ and $N_E > N_{E'}$ it follows that $p$ must occurr in one of the pairs in~$\calM_E$ (which is a finite set).

For each $p$ in a pair of $\calM_E$ we check that there is no congruence with an elliptic curve~$E'$ of smaller conductor.
To speed up the computation we note that the conductor of~$E'$ must 
be of the form~\eqref{E:condE'} where $k' = k$ when 
\[
k = 2 \quad \text{ or } \quad k=1  \text{ and } p \nmid \vv_{p}(\Delta_E)
\]
or $k' \in \{0,1\}$ when 
\[
k = 0 \quad \text{ or } \quad k=1  \text{ and } p \mid \vv_{p}(\Delta_E).
\]
Observe that if $k=0$ and $k'=1$ we are forced to have a prime~$q_i'$ dividing $N_{E'}$ that disappears mod~$p$ to keep the condition $N_E > N_{E'}$.

After running $E'$ through all the curves with conductor smaller than $N_E$ we pick $E$ to have the next smaller (or equal) conductor. At each step there are less cases for~$E'$.
...
@incollection {MR1363500,
    AUTHOR = {Rubin, K. and Silverberg, A.},
     TITLE = {Families of elliptic curves with constant mod {$p$}
              representations},
