...
Then \textbf{(with some extra hypotheses!)} the image of~$\rhobar_{E,p}$
...
\textbf{In case $p=3$ the rest of the statement needs more work since
  ...
  we need to make clear that Case (2) applies, not Case 1.}
...
$P_1, P_2$ of $E[p]$ such that the image of~$\rhobar_{E,p}$ is
...
%% \end{pmatrix} \quad \text{ if } \eps_d(\sigma) = -1.  \\
%% \end{cases}\]
Fix $\tau \in G_K$ such that \textbf{[this is possible if $G_K$ maps
    onto $N$; what if not?]}
\[
...
\phi(\sigma(P))$ for all $\sigma \in G_K$ and $P \in E[P]$. We first
consider the acton of~$\tau$.  Write $P_i = (x_i,y_i)$. Since
$\eps_d(\tau) = -1$, we have
\[
...
\begin{example}
  [\textbf{needs completion}] $E_1=\lmfdbec{32}{a}{1}$ has
  $j$-invariant $1728$, its $2$-isogenous twist is
  $E_1^{-1}=\lmfdbec{32}{a}{2}$, and these are antismplectically
  ...
  $5$-congruent to each other in accordant with Theorem~\ref{T:Cartan}
  \textbf{[check this]}.  The mod~$5$ congruence between
  $\lmfdbec{32}{a}{1}$ and $\lmfdbec{608}{b}{1}$ is ? while that
  between $\lmfdbec{32}{a}{2}$ and $\lmfdbec{608}{e}{1}$ is ??
  \textbf{[find and insert these, there should be one of each type]}, so
  in this case applying the $-1$-twist reverses the symplectic type.
\end{example}
