...
give rise to $\Q$-points on at least one of the modular curves~$X_E(p)$ or $X_E^-(p)$. 
\jc{I put back ``at least'' here since just saying ``one of'' implies
  that a triple never gives a point on both, but that would only be
  true if we excluded triples whose symplectic type is not
  well-defined, which is not the case at this exact point.}
...
\jc{Added the previous sentence}
...
\jc{Inserted previous two sentences.}
...
\jc{I deleted ``$u \neq -4$ modulo~$4$-th powers and `` since if $u$
  is -4 times a 4th power then it is -1 times a square!}
...
In the non-split case, we have shown that equaton~\eqref{E:PsiM} holds
over $\GL_2(\F_{p^2})$. By undoing the initial change of coordinates
...
\]
for all $\sigma\in G_K$, giving ~\eqref{E:PsiM}.
...
\jc{You will have to explain to me what is not clear about these
  congruences!  I think the statement is completely correct as it is
  and that the lines below prove the congruences exactly as stated.}
