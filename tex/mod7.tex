...
\begin{proof} 1) Let $E'$ be a $p$-congruent twist of~$E$. If $E'$ is
  ...
  (not defined over~$K$, since the proective image is not abelian) by
...
Proposition~\ref{P:twist}), this is a situation where the local
...
[\def\rnu{\root n\of u} \def\rnub{\root n\of {\overline{u}}} Write
  ...
  In the second case, we have $\tau(\rnu)=\rnub$ (without loss); using
  $\tau\sigma\tau=\sigma^{-1}$ gives $\sigma(\rnub)=\zeta\rnub$.
  Hence $v=\rnub/\rnu\in K'$, and $\overline{u}=uv^n$.  Conjugating
  imples that $(v\overline{v})^n=1$ and hence $v\overline{v}=\pm$,
  and even $v\overline{v}=1$ for $n=3$ since the only $n$th roots of
  unity in~$K$ are (by hypothesis $K'\not=K$)~$\pm1$ for $n=4,6$ and
  just~$1$ for $n=3$.

  In case $v\overline{v}=1$ we have $v(1+\overline{v})=1+v$ and it
  follows that $u_1=u(1+v)^n\in K$.  Replacing $u$ by $u_1$ completes
  the argument in this case.

  If $v\overline{v}=-1$ then we cannot replace $u$ by an eleent
  of~$K$.  For if $u_1=uw^n\in K$ then
  $v^n=(w/\overline{w})^n)\implies v=\zeta^kw/\overline{w}\implies
  v\overline{v}=1$ (for some~$k$), while if $u_1=u^{-1}w^n\in K$ then
  $u\overline{w}^n\in K$ and we again deduce that $v\overline{v}=1$.

  The formula for the norm from $K'$ to~$K$ gives
  $-1=v\overline{v}=x^2+y^2$ or $x^2+3y^2$ for some $x,y\in K$ for
  $n=4$, $n=6$ respectively.  Hence it is impossible to have
  $v\overline{v}=-1$ unless $K$ is totally complex; in particular our
  claim is true over~$\Q$.
]

\jc{That is as far as I got (note that I have made some corrections
  since last time).  I also thought that the rest of the proof would
  work OK if we allowed $u\in K'$ (not in~$K$).  But no: we rely on
  $\rnu$ being fixed by some $\tau$ of order $2$ in the other coset!
  However, I have an alternative for the last part which does not
  require this: see below.}
...
\GL_2(\F_p)$, to do so for a single element~$\tau\in G_K\setminus
G_{K'}$.

\jc{First version, fine when $u\in K$}
...
\jc{New version which does not rely on the existence of a $\tau\in
  G_K\setminus G_{K'}$ fixing $\rnu$:}

Let~$\tau\in G_K\setminus G_{K'}$ be arbitrary, and write
$\Psi(\tau)=\diag(w,w^{-1})$.  Define
\[
...
of~$C$ occurs in~$\GL_2(\F_p)$ and so~\eqref{E:PsiM} holds with $A \in
\GL_2(\F_{p})$.  In the non-split case, we have shown that
equaton~\eqref{E:PsiM} holds over $\GL_2(\F_{p^2})$. By undoing the
...
matrices with respect to an $\F_p$-basis, and let the change of basis
...
\item $C^{-1}\Psi(\sigma)C = \diag(\eps(\sigma),\eps(\sigma)^{-1})$
  for $\sigma\in G_{K'}$.
\end{itemize}
Let $B=\rhobar_{E,p}(\tau)\in\GL_2(\F_p)$, so that $A=C^{-1}BC$ is
...
\jc{The rest of the proof is still only for the case $u\in K$.  As
  already observed, this may always be assumed unless (possibly) $K$
  is toally complex.}
...
Let $K=\Q$, $p=3$, $j=1728$ and write $E_a := E_{a,0}:\ Y^2=X^3+aX$,
$a \in \Q^*$.  The mod~$3$ projective image is the normaliser of a
non-split Cartan, isomorphic to~$D_4$, since the projective division
field~$F$ is obtained by adjoining the roots of the $3$-division
polynomial $3X^4+6aX^2-a^2$, and $F=\Q(\sqrt{-1},\root4\of u)$
where~$u=-a^2/3$ and it is not hard to see that $a/\sqrt{-3}$ is not a
square in $\Q(\sqrt{-1},\sqrt{-3})$.  The roots of the polynomial are
$\frac{1-\sqrt{3}}{1-i}\root4\of{u}$ and its conjugates.

Following the proof of Proposition~\ref{P:higherTwists}, one checks
that up to $4$th powers, the subgroup of $\Q^*/(\Q^*)^4$ which become
$4$th powers in~$F$ is generated by~$-3/a^2$ and~$-4$: the maps
$\Q^*/(\Q^*)^4 \to \Q(\sqrt{-1})^*/(\Q(\sqrt{-1})^*)^4$ and
$\Q(\sqrt{-1})^*/(\Q(\sqrt{-1})^*)^4 \to F^*/(F^*)^4$ have kernels
generated by~$-4$ and~$u$ respectively.

Hence, from Proposition~\ref{P:higherTwists} we expect a
$3$-congruence between $E_a$ and either $E_{-3/a}$ (twisting
by~$u^{-1}=-3/a^2$) or $E_{-a^3/3}$ (twisting by~$u$).  Only the first
holds.
