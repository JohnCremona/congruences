...
$F=\Q(\sqrt{d})$. Then $E[p] \simeq E^d[p]$ for all primes $p \geq 5$  unramified 
in~$F$ and of good reduction for~$E$. 
Moreover, for each such~$p$, $E^d$ is the unique quadratic twist congruent to~$E$.
\end{corollary}
\begin{proof} 
...
Moreover, $\PP \rhobar_{E,p} \simeq D_n \neq C_2 \times C_2$ (since $p \geq 5$) and $\PP \rhobar_{E,p}(G_F) \simeq C_n$. Thus, for each such~$p$, in the notation of Proposition~\ref{P:twistNumber}, we have $M=F$ and $M$ is unique, hence~$d$ modulo squares is unique.
\end{proof}
...
Let $E/K$ and $E'/K$ be elliptic curves over a number field~$K$ which are quadratic twists, say $E' = E^d$ for some $d \in
...
twists of each other: the quartic twist of $E_a$ by $t$ is $E_{ta}$, which is isomorphic to $E_a$ if and only if $t$ is a $4$th power. 

Each $E_a$ has CM by $\Q(\sqrt{-1})$. If $-1$ is not a square in~$K$ then
$E_a$ and~$E_{-4a}$ are not isomorphic but are $2$-isogenous (Silverman~??), and hence $E_a[p]\cong E_{-4a}[p]$ for 
all~$p>2$. If $-1$ is a square in $K$,  then $-4=(1+\sqrt{-1})^4$ is a 4th power and 
$E_a$ and $E_{-4a}$ are isomorphic; 
moreover, in this case, we have 
$\End_K(E_a)\cong\Z[\sqrt{-1}]$ and the mentioned isogeny is the
endomorphism~$1+\sqrt{-1}$.  

Our result for these curves is that there are no other isomorphisms
between their $p$-torsion for any~$p>2$, provided that the image of
...
Conversely, if $E_{a_1}[p]\cong E_{a_2}[p]$ then $a_1/a_2 \in\{1,-4\}$
modulo~$(K^*)^4$, so there are no other mod~$p$ congruences between
these curves. \textbf{We need an assumption on the mod~$p$ image being
  not too small here}
\end{theorem}

\begin{proof}
As already remarked, the first part follows from the fact that $E_a$
and~$E_{-4a}$ are $2$-isogenous, together with the isogeny criterion.

As the curves are quartic twists, we have that
$a_1/a_2 = \alpha^4$ with $\alpha$ at most in 
a quartic field. 
We have two possibilities:

(a) We have $a_1/a_2 = d^2$ in~$K$. Here  
the curves are quadratic twists by~$d \in K$
and we can take $\alpha = \sqrt{d}$ and the 
curves are isomorphic over~$\in K(\sqrt{d})$.

(b) We have that $a_1/a_2$ is not a square in~$K$.  
Let $m=\sqrt{a_1/a_2}$ and
set $L=K(m)$, a quadratic extension of~$K$.  
Thus $a_1/a_2 = \alpha^4 = m^2$, we can take 
$\alpha = \sqrt{m}$ and the curves $E_{a_1}/L$ and~$E_{a_2}/L$ are quadratic twists by~$m$.

Suppose that $E_{a_1}[p]\cong E_{a_2}[p]$ but that $E_{a_1}$ and $E_{a_2}$ are not isomorphic over~$K$. 

Suppose $E_{a_1}$ and~$E_{a_2}$ become isomorphic over some quadratic extension $K(\sqrt{d})$ of~$K$, 
that is $a_1/a_2 = \alpha^4$ with $\alpha \in K(\sqrt{d})$. 
This is is possible in two ways :

(i) We have $a_1/a_2 = d^2$ in~$K$ (i.e. case (a) above). Here  
the curves are quadratic twists by~$d \in K$ and
this implies that 
$\rhobar_{E_{a_1},p} \simeq \rhobar_{E_{a_1},p} \otimes \chi_d$, where $\chi_d$ is the character 
corresponding to~$K(\sqrt{d})$.

Applying Proposition~\ref{P:twist}, the image of $\rhobar_{E_{a_1},p}$ is
...
extension $K(\sqrt{d})/K$.  By hypothesis \textbf{NB: not yet stated}
this is true for a unique pair $(N,C)$; hence the quadratic extension
cut out by $N/C$ must also be the CM extension $K(\sqrt{-1})$.  Note
that we cannot have $\sqrt{-1}\in K$ in this situation, since then the
image would be contained in a Cartan and it would not be the case that
half the traces were~$0$ (see the proof of Proposition~\ref{P:twist}).
Hence $K(\sqrt{d})=K(\sqrt{-1})$ is quadratic over~$K$,
\begin{comment}
{\bf I don't understand this paragraph :}
...
\end{comment}
and therefore $d=-s^2$, $s \in K$ and so $a_1/a_2 = d^2 = s^4$, thus $E_{a_1}$ and~$E_{a_2}$ are isomorphic over~$K$. 


(ii) We have $a_1/a_2 = \alpha^4$ in $K(\sqrt{d})$
but $a_1/a_2$ is not a square in~$K$. 
Here the curves are quadratic twists in $K(\sqrt{d})$ which are also isomorphic over this field; note this is the case in the example mentioned above: $-4$ is
not a square in~$\Q$ but equals the $4$th power $(1+\sqrt{-1})^4$ over~$\Q(\sqrt{-1})$. In this setting, 
we only know the weaker condition on the representations
$\rhobar_{E_{a_1},p}|_{G_{K(\sqrt{d})}} \simeq \rhobar_{E_{a_2},p}|_{G_{K(\sqrt{d})}}$, so we cannot apply Proposition~\ref{P:twist} directly.


%If $a_1/a_2 = d^2$ with $d\in K^*$ not square, then the curves are
%quadratic twists by~$d$, so $K(\sqrt{d})=K(\sqrt{-1})\not= K$.  But
%then $-d\in(K^*)^2$, so $a_1/a_2=(-d)^2\in(K^*)^4$, contradiction.

Let $m=\sqrt{a_1/a_2}$ and
set $L=K(m)$, a quadratic extension of~$K$.  
Since $ a_1/a_2 = m^2$ in~$L$, 
$E_{a_1}[p]\cong E_{a_2}[p]$ as $G_L$-modules, and $E_{a_1}$ and~$E_{a_2}$ are quadratic twists by~$m$ (in particular isomorphic over~$L(\sqrt{m})$) we can apply the argument in~(i) to $E_{a_1}/L$ and $E_{a_2}/L$ {\bf (provided the image condition still holds)} and conclude they are isomorphic over~$L$. This means that $a_1/a_2 = m^2 = m_0^4 $ with $m_0 \in L$.

{\bf But then they are quadratic twists over~$K$}, so $L=K(\sqrt{-1})$.

It remains to show that $a_1/a_2=-4$ (modulo $4$th powers).  Since
$L=K(\sqrt{-1})$ we have that $a_1/a_2$ is minus a square, say
$a_1/a_2=-4s^2$ with $s\in K$.  We already know that $a_1/a_2\in(L^*)^4$.  Hence
$s^2=t^4$ with $t\in L$; here we use the fact noted above that $-4$ is
a $4$th power in $K(\sqrt{-1})$.  Therefore, $\pm s=t^2$ is a square in~$L$, 
but~$L=K(\sqrt{-1})$ {\bf so $\pm s$ is a square in~$K$}
and so $a_1/a_2$ is indeed $-4$ times a $4$th power in~$K$.

{\bf What happens to the case where the curves are not isomorphic over a quadratic field? This is a subcase of (b) above.}
