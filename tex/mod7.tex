...
and~$'$ respectively.  This test can very quickly establish rigorously
...
It is also possible to prove positive results this way using only a
finite number of primes~$\ell$, but this takes some care.

We first outline the steps briefly, and then consider each in more detail.
\begin{enumerate}
...
\item For each reducible subset~$S$, determine whether, for each pair
  $E$, $E'$ chosen as before, there is an isomorphism between $E[p]$
  and $E'[p]$ (and not just between their semisimplifications), if
  necessary replacing one of $E$, $E'$ with another curve
  $p$-isogenous to it.  If so, process as in the previous step.
\end{enumerate}
...
For an integer~$B\ge1$, let $\calL_B$ be the set of the smallest~$B$
...
\ell\in\calL\}$.  For example we may enumerate
$\calL=\{\ell_0,\ell_1,\dots,\ell_{B-1}\}$ and use the value
...
for all~$\ell\in\calL_B$.

Example: after running step 1 using $B=30$ for $p=7$, of the $20138$
...
genuine triple of curves which have mutually isomorphic
$\mod{7}$-representations.

This false positive arises since \lmfdbec{25921}{a}{1} and
\lmfdbec{78400}{gw}{1} have $a_{\ell}$ which are congruent modulo~$7$
for all~$\ell\in\calL_{30}$.  In this case, using $\calL_{40}$ instead
would have eliminated this, but then Step~1 would have taken longer.

\subsection{Testing reducibility}
For each set (of size greater than one) of isogeny classes of curves
...
Example: of the 20138 nontrivial sets from Step~2 we find that 19883
are irreducible ({\it i.e.,} consist of curves whose
$\mod{7}$-representations are irreducible, while 255 are reducible.

The sizes of these sets are as follows.  The irreducible sets have
size at most~$5$;  most (18334) have size~$2$.  The reducible sets
have size up to~$76$.



[I could include a table of all these sizes if we think it is
  sufficiently interesting.  The frequencies for each size are

Irreducible  {2:  18334, 3: 1297, 4: 246, 5: 6}
...
\subsection{Distinguishing symplectic from antisymplectic: irreducible
  case}

After the previous step we have a collection of pairs of isogeny
classes of curves $E,E'$ such that $E[p]\cong E'[p]$ with $E[p]$
irreducible as $G_{\Q}$-module, and wish to determine whether this
isomorphism is symplectic or anti-symplectic.  For all odd~$p$ we may
apply the local criteria of \cite{FKsym}, while for $p=7$ we use the
...
$\phi:E[7]\to E'[7]$ is an isomorphism of symplectic $G_K$-modules; we
...
$(E,\id)$.  In \cite{Halberstadt-Kraus-XE7} one can find an explicit
formula for the rational function $j:X_E(7)\to X(1)$, and also
polynomials $c_4,c_6\in\Z[a,b][X,Y,Z]$ of degree~$20$ and~$30$
respectively such that for all but finitely many points $P=(x:y:z)\in
X_E(7)$ the associated curve $E'$ has model
$Y^2=X^3-27c_4(P)X-54c_6(P)$.  The finitely many common zeros of $c_4$
...
four such pairs $(c_4,c_6)$, of which the first is the pair in
\cite{Halberstadt-Kraus-XE7}, and such that at each point $P\in
X_E(7)$ at least one pair $(c_4(P),c_6(P))\not=(0,0)$, thus supplying
us with a model for the isomorphic curve $E'$ at each point~$P$.

We make use of this model and formulas as follows, given a pair of
curves $E,E'$ with $E[7]\cong E'[7]$.  Using one curve~$E$ we write
down the model for $X_E(7)$.  Then we find all preimages (if any) of
$j(E')$ under the map $j:X_E(7)\to X(1)$.  While in principal (over an
algebraically closed field) there may be as many as $168$ preimages,
over $\Q$ there is never more than one (assuming that $E'[7]$ is
...
explicit formulae for the $j$-map and $c_4,c_6$ as above in this case
in \cite{Fisher}.

The reader will have observed that it is not necessary to apply both
the symplectic and antisymplectic tests to a pair $(E,E')$ if we know
already that $E[7]\cong E'[7]$, since (in this current irreducible
case) one will succeed if and only if the other fails.  However we did
apply both tests in our computations with the curves in the database
as a test of our implementation, verifying that precisely one test
passes for each pair.  We also checked that the results obtained for
each pair using the local criteria are the same, so that we can be
confident in the correctness of the results.
...
image of the $\mod{7}$ representation~$\rho_{E,7}$ attached to~$E$ is contained in
a split Cartan subgroup of $GL(2,\F_7)$, and this cannot occur
over~$\Q$ (see \cite{Sutherland}).  The size of the isogeny class of
$E$ is (over $\Q$) either $2$, consisting of two $7$-isogenous curves,
...
Fix one such curve $E$ with $E[7]$ reducible.  The image
of~$\rho_{E,7}$ has the form
\[
  \begin{pmatrix}\chi_1&*\\0&\chi_2  \end{pmatrix},
\]
where~$\chi_1$, $\chi_2$ are characters $\G_{\Q}\to\F_7^*$ and the $*$
component is nontrivial.  Moreover $\chi_1\chi_2$ is the cyclotomic
...
E'[7]^{\sss}$.  The image of $\rho_{E',7}$ has the form
\[
  \begin{pmatrix}\chi_1'&*'\\0&\chi_2'  \end{pmatrix},
  \]
...
kernel of the unique $7$-isogeny from~$E$.  In this way we can
determine whether $\chi_1=\chi_1'$ and $\chi_2=\chi_2'$ or vice versa.
In the second case, we replace $E'$ with its $7$-isogenous curve,
which has the effect of interchanging $\chi_1'$ and $\chi_2'$ (as well
as changing $*'$).  Now the image of $\rho_{E',7}$ has the form
\[
\begin{pmatrix}\chi_1&*'\\0&\chi_2  \end{pmatrix},
\]
with the same characters, in the same order as for $\rho_{E,7}$, and
...
Although the maps $*,*':G_{\Q}\to\F_7$ are not homomorphisms, it is
...
of the eight $7$-isogenies from~$E$.  In this case, it has a single
...
In this way, for each pair $(E,E')$ whose $\mod{7}$-representations
