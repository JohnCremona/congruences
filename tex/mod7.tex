...
$t:E\to E'$ defined over $\Kbar$, so that\footnote{We use left group
  ...
  $\sigma(t(P))={}^{\sigma}t(\sigma(P))$.}
for $\sigma\in G_K$ we have
...
and~$E_{au,0}$.  We have ${}^{\sigma}t = \psi(\sigma)t = t\psi(\sigma)$
...
and~$E_{0,bu}$.  Again we have ${}^{\sigma}t = \psi(\sigma)t =
...
\item Let $E'$ be the twist of~$E$ by a cocycle~$\psi$. Then there is
  ...
  degree at most~$6$, satisfying
\begin{equation}\label{E:iso-twist-general}
\sigma(t(P)) = \psi(\sigma)t(\sigma(P)) %%% = t(\sigma(P))\psi(\sigma)
\qquad \text{for all~$\sigma\in G_K$ and $P\in E(\Kbar)$}.
  \end{equation}
\item
  In the case of a quadratic twist with $E'=E^d$, the isomorphism $t
  :E\to E^d$ is defined over $K(\sqrt{d})$ and satisfies
...
twist is not quadratic.  Instead we have the matrix
version~\eqref{E:PsiMatrix}.
\end{enumerate}
\end{lemma}

\begin{proof}
Equations and (\ref{E:iso-twist-general}) was given above and
(\ref{E:iso-twist}) is a special case.  Relation (\ref{E:rho-twist})
follows immediately from (\ref{E:iso-twist}); in matrix form this is
just~\eqref{E:PsiMatrix} since $\Psi_{\sigma}=\varepsilon_d(\sigma)I$.
...
of the relation~(\ref{E:rho-twist}) we have the matrix
version \eqref{E:PsiMatrix}.
\end{proof}
...
%Our objective in this section is to classify the symplectic type of
%congruences between twists; this only makes sense if the $p$-torsion
%modules of the relevant curves do not admit anti-symplectic
%automorphisms, \textit{i.e.}, they satisfy \condS\ of the
%introduction.
...
Under the natural \condS, the previous lemma tells us we
...
As examples of the exceptions in the previous lemma, we have the
following:
...
\jc{I changed $M$ to $A$ since we have just been using $M$ to denote a
  field.  I don't think we have used $A$ for anything else!}
...
where~$\Psi_\sigma$ is as in~\eqref{E:PsiMatrix} is a homomorphism
...
$F'$ contains the projective division field~$F$.
...
when $n$ is even.  The fixed field of its kernel is $F/K'$, so
$\Gal(F/K)\cong D_2$ or~$D_3$.

\jc{Now I seem to have reached the conclusion that the projective
  image of $G_K$ is either $D_2$ or $D_3$.  What is wrong?!}
...
\subsection{John's new way of thinking\dots}

Let $j(E)=0, 1728$ and $p\not=3$ in case $j=0$ so the projective image
is dihedral $D_n$ for some $n\ge2$.  Let $K'=K(\sqrt{-3})$ or
$K(\sqrt{-1})$ respectively, so the projective image of $G_{K'}$ is
$C_n$.

Over $\overline{\F}_p$ we can diagonalise the $C_n$, so for $\sigma\in
G_{K'}$ we have
\[
...
cocycle $\psi$.  The matrices $\Psi_\sigma$ are now also diagonal, say
\[
...
[Explicitly: $\psi(\sigma)\in\Aut(E)\cong\calO^*$ with a fixed (once
  and for all) isomorphism $\Aut(E)\cong\calO^*$.  In the split case
  ...
  $\calO^*\to\F_p^*$.  In the non-split case $p\calO=\frp$ and
  $\eta(\sigma)$ is the image of $\psi(\sigma)$ under the isomorphism
  followed by the reduction $\calO\to\calO/\frp\cong\F_{p^2}$ which
  induces a reduction homomorphism $\calO^*\to\F_{p^2}^*$.]

Now suppose that $E$ and $E'$ are $p$-congruent.  By \eqref{E:PsiM} we
have, for $\sigma\in G_{K'}$, after cancelling the scalar
factor~$c_\sigma$ from each side,
\[
...
\]
and hence $\eps(\sigma)=\eta(\sigma)$ for all $\sigma\in G_{K'}$.
Hence $m=n$ and the projective image is $D_n$ where $n\in\{3,4,6\}$ is
the order of the twist.

\jc{So far this is another way of proving things we have other proofs
  for, but is possibly simpler.  However it gets more interesting now.}

Conversely, let $E$ and $p$ be as above with projective image~$D_n$
where $n=4$ if $j(E)=1728$ and $n\in\{3,6\}$ if $j(E)=0$.  Then the
projective $p$-division field is $F=K'(\root n\of u)$ for some $u\in
K^*$, unique up to multiplication by an element of~$K^*\cap(K'^*)^n$ or
replacing $u$ by~$u^{-1}$, the latter since $(\Z/n\Z)^*=\{\pm1\}$.  Up
to $n$th powers in~$K$ we have either four or two possible choices
for~$u$, replacing $u$ by one of
\begin{itemize}
...
$F/K'$.  \emph{Replacing $u$ by $u^{-1}$ if necessary}, comparing
characters gives
\[
\eps(\sigma) = \eta(\sigma)
\]
where (as above) $\eta(\sigma)$ is the image of
\[
\sigma(\root n\of u)/\root n\of u \in \mu_n(K')\subseteq\calO^*
\]
under the reduction map $\calO^* \to\overline{\F}_p^*$.

Now let $E'$ be the order~$n$ twist of~$E$ by~$u$, with cocycle $\psi$
having matrix $\Psi_{\sigma}$.  For any anti-diagonal matrix~$A$, the
same computation as before gives
\[
A\ \diag(1,\eps(\sigma))\ A^{-1} = \diag(\eps(\sigma),1) =
\diag(1,\eps(\sigma)) \diag(\eta(\sigma),\eta(\sigma)^{-1}).
\]
and hence \eqref{E:PsiM} holds for $\sigma\in G_{K'}$.  To show that
\eqref{E:PsiM} holds for all $\sigma\in G_{K}$, it suffices, since
both sides are homomorphisms $G_K\to \GL_2(\F_p)$, to do so for a
single $\tau\in G_K\setminus G_{K'}$.

Let $\tau\in G_K$ be an automorphism of order~$2$ such that its
restriction to $K'$ is non-trivial but $\tau$ fixes~$\root n\of u$.
Then $\tau\notin G_{K'}$, and $\psi(\tau)=1$ so $\Psi_\tau=I$.
Setting $A=\rhobar_{E,p}(\tau)$, \eqref{E:PsiM} is trivially
satisfied.

Since this construction only works for one out of each inverse
pair~$u,u^{-1}$ we have thus constructed two quartic or sextic twists
when $n=4$ or $n=6$ respectively, and just one cubic twist when $n=3$.
The two quartic twists differ by the quartic twist by~$-4$, so by a
$2$-isogeny, while the two sextic twists differ by the sextic twist
by~$-27$, so by a $3$-isogeny.
...
Hence if $\sqrt{p^*}\in K$ these congruences between higher order
twists are always symplectic.  If $\sqrt{p^*}\notin K$ then
$K(\sqrt{p^*})$ is a quadratic subfield of~$F$ and hence either
$K(\sqrt{p^*})=K(\sqrt{u})$ or (when $n=4$)
$K(\sqrt{p^*})=K(\sqrt{-4u})$ or (when $n=6$)
$K(\sqrt{p^*})=K(\sqrt{-4u})$.  The congruence is symplectic if and
only if $K(\sqrt{p^*})=K(\sqrt{u})$; that is, if and only if $u$ is
$p^*$ times a square.

Remark: when $n=4,6$ the two quartic twists are $2$-isogenous
(respectively $3$-isogenous) to each other and are antisymplectically
congruence to each other by the previous argument.  It follows that the
hypothesis that the projective image is $D_4$ (respectively $D_6$)
implies that $2$ is a quadratic residue of~$p$, \textit{i.e.}
$p\equiv\pm1\pmod8$ (respectively, $3$ is a quadratic residue of~$p$,
\textit{i.e.}  $p\equiv\pm1\pmod{12}$).

To summarize: for the cases $n=4,6,3$ there are respectively $2,2,1$
congruences from $E$ to a twist by~$u$ (for some $u$) of order~$n$.
If $\sqrt{p^*}\in K$ these congruences are all symplectic.  If
$\sqrt{p^*}\notin K$ the congruences are symplectic if and only if
$n=4$ or~$6$ and $u=p^*$ (modulo squares in~$K$), and for $n=4,6$ one
congruence is symplectic and the other anti-symplectic.

 \jc{this agrees with Theorem~\ref{T:cubicM} and with
   Theorem~\ref{T:quarticM} for quartic twists and
   Theorem~\ref{T:sexticM} for sextic twists}

\jc{Examples moved from Section 3 with essentially no change except to
  give the symplectic type}
...
\jc{numbers in this example need updating for $N\le500000$}
...
in just 6 cases, and in the remaining 55 cases, only one congruence is
symplectic; and~$10$ of size~$4$, in all of which there are two pairs
of symplectically congruent curves while the congruences between
curves in different pairs is anti-symplectic.  In all, therefore,
there are 556 symplectic pairs, of which 224 are symplectic and 332
antisymplectic. \jc{I added a little more detail here than before}
