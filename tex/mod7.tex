...
of~$\rhobar_{E,p}$, so covers cases (i), ~(ii) entirely and case (v) with $p=11$.
...
\nf{Maybe we should move the paragraph above about which cases our method covers to here? This should highlight more what is our contribution: i.e. among the cases left it covers essentially all of them. }
...
arise in a number of ways in our study; these are often between quadratic twists but they also occur between higher order twists. In this sectoin we study all types of congruences in detail.
...
isomorphic to~$D_{n}$ for some~$n \geq 2$ (see~\cite[Theorem~XI.2.3]{LangModForms}). The preimage of~$C$ in~$G_K$ then cuts out a quadratic 
extension~$M = K(\sqrt{d})$ satisfying 
$\PP \rhobar_{E,p}(G_M)
\simeq C_n$.  We will see in Proposition~\ref{P:twist} that in
this setting there is a mod~$p$ congruence between~$E$ and its
quadratic twist by~$d$. This  construction is the main source
of congruences between twists and it appeared already in~??;
our main contribution for this type of congruences is then to describe their symplectic type in Theorem~\ref{T:quadratic}. The second contribution of this section is to classify congruences between higher order twists in Proposition~\ref{P:higherTwists} 
and describe their symplectic
type in~Theorem~\ref{T:typeHigher}.
...
$\Psi$ matches the quadratic character~$\varepsilon_d$ associated to the
...
is dihedral of order at least~4; equivalently {\bf (unless the image is
...
\begin{example} \label{Ex:3twists}
  ...
  \nf{Don't you mean Cremona label?}
...
$p\not=3$. 
\end{itemize}
\nf{I think we need these extra assumptions on~$u$ for defining~$A$ in the proof of Prop.~\ref{P:higherTwists}, otherwise we have to consider these cases within the proof; see changes there for details. I don't think we need to add that~$u$ is not a 4th or 6th power because this is implied by the hypothesis in the results that follow.}
...
\end{proof}
\nf{Isn't $c_\sigma$ a character? Wouldn't be better a notation looking more like a character? $c(\sigma)$?}
\begin{lemma}\label{L:DiagonalPsi}
Let $E'$ be the order~$m\in\{3,4,6\}$ twist of~$E$ by $u\in K^*$, with
cocycle $\psi$. Fix basis 
so that~\eqref{E:PsiMatrix} holds.
Then, after extending scalars to~$\Fbar_p^*$ and changing basis so that  
\eqref{E:diagonalC} holds, 
the matrices $\Psi_\sigma$ in~\eqref{E:PsiMatrix} become diagonal. 
More precisely,
\[
...
of~$G_{K'}$, which is not scalar. Note that the basis giving~\eqref{E:diagonalC} diagonalizes the whole of~$C$, therefore~$\Psi_\sigma$ is diagonal. Finally,  
...
\nf{eliminated reference to~$E'$ in the above proof. Did I overlooked something?}

\begin{lemma}\label{L:EpsEta}
  Keeping the notation of Lemmas~\ref{L:diagonalC} and~\ref{L:DiagonalPsi}, suppose also that $E$ and $E'$ are $p$-congruent.
  Then $\eps(\sigma)=\eta(\sigma)$ for all $\sigma\in G_{K'}$.  Moreover, 
  we have $\PP \rhobar_{E,p}(G_K) \simeq D_n$
  with $n=m \in \{3,4,6\}$  
   is the  order of the twist.
\end{lemma}
\begin{proof}
...
diagonal by~$A$ just interchanges the two diagonal entries, so
...
By Kummer theory the extension $F/K'$ is determined by a specific cyclic subgroup
of order~$n$ in~$K'^*/(K'^*)^n$
hence, noting that $\Aut(C_n)\cong\{\pm1\}$ for $n\in\{4,3,6\}$, we conclude 
that~$u \in K^*$ is unique up to
replacing $u$ by~$u^{-1}$ and multiplication by an element
of~$K^*\cap(K'^*)^n$.
...
We can thus choose $\tau\in G_K$ such that it fixes~$\root n\of u$ and is non-trivial when restriced to~$K'$. 
...
\nf{Maybe it is better to include more details proving of one of the cases above? I don't find this so clear. For example, why aren't there other elements of~$K'$ whose~$n$th power is in $K$?}
...
Indeed, we have choosen $\tau\in G_K$ to be such that it fixes~$\root n\of u$ and 
is non-trivial when restriced to~$K'$ and 
...
(b) Suppose $n=4$. We have $\Gal(F/K) \simeq D_4$ and there are exactly three quadratic subextensions of~$F/K$.
...
(c) Suppose $n=6$. We have $\Gal(F/K) \simeq D_6$ and again there are exactly three quadratic subextensions of~$F/K$.
Furthermore, the fields 
...
\nf{I am confused by the following paragraph. Why do we give the congruence conditions? Can you rewrite their proof?}
...
\nf{As above I think we should keep only one of the following two examples. Similar comments as above apply.}
...
\nf{Shouldn't this section be a subsection of \S4 instead or even be inside \S4.5 ?}
