...
(1) Assume first that either $d\not=-1$, or that both $b_1$ and~$b_2$
are non-zero, so the quadratic twists have equations
$E_k^d:Y^2=X^3+a_kd^2X+b_kd^3$.  (See Lemma~\ref{L:quadratic-twist}.)

An isomorphism $E_k\to E_k^d$, defined over~$K(\sqrt{d})$, is given by
...
fact $\psi$ is $G_K$-equivariant.

(2) Now suppose that $d=-1\notin(K^*)^2$, and that one of the curves,
...
Despite this result, it is hard to say precisely how many congruences
...
\begin{example} \label{Ex:3twists}
  ...
  $E^{-3}$. \textbf{NF: please check I have this the right way round.}
\end{example}
\begin{example} \label{Ex:LocalFail7}
Consider the elliptic curve
\[ E : y^2 + y = x^3 - x^2 - 74988699621831x +  238006866237979285299, \]
which has conductor $ N_E = 7^2 \cdot 2381 \cdot 134177^2>2\cdot10^{15}$, so
is not in the LMFDB database.  The image of the mod~$7$ Galois
representation is equal to the normalizer of a non-split Cartan
subgroup.
