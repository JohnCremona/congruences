...
Note that $N = N'$ because $E[p] \simeq E'[p]$ as $G_\Q$-modules.

{\bf Assume for now that both curves have good reduction at $p$ so that $N_E$ and $N_{E'}$ coincide with its part away from~$p$.}

We have $N_p \mid N_E$ and from Kraus~??
it follows that, for each $\ell \neq p$, if 
$\vv_{\ell}(N_p) \neq \vv_{\ell}(N_E)$ then
$\vv_{\ell}(N_E) = 1$ and $p \mid \vv_{\ell}(\Delta_E)$;
similarly for~$E'$.

In particular, if $N_E \neq N_p$,
then there are primes $q_0,\cdots,q_n$ such that 
$N_E = p^0 \cdot N\cdot q_0 \cdot \ldots \cdot q_n$ 
and $p \mid \vv_{q_i}(\Delta_E)$ for all $i$. 

Fix a concrete curve $E$ in the LMFDB. Let $\calM_E$ be the set of pairs $(q,p)$ where $q$ is a multiplicative prime of~$E$ and
$p \geq 19$ is a prime satisfying $p \mid \vv_{q}(\Delta_E)$.
In particular, if $N_E > N_{E'}$ then $p$ must occurr in one of the pairs in~$\calM_E$.



A similar set of primes~$q_i'$ exists for $E'$.
