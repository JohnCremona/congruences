...
equations, together with $X_E^-(7)$ and $X_E^{\pm}(11)$ were found by Fisher in~\cite{Fisher}. Equations for $X_E^{\pm}(3)$
and $X_E^{\pm}(5)$ are classical (see~??).
...
\begin{proposition}
Let $E/\Q$ and $E'/\Q$ be elliptic curves which are non-isomorphic quadratic twists, say $E' = dE$ for some $d \in \Q^*$. Assume that $E[p]\cong E'[p]$ with $p\ge3$. 

Then the image 
of~$\rhobar_{E,p}$ is contained in the normaliser of a Cartan subgroup of $\GL_2(\F_p)$ but not in the subgroup itself. 
Moreover, there are either 1 or 3 values of~$d$ with this property, where the latter case occurs when $\rhobar_{E,p}$ has projective image 
$\Z/2\Z \times \Z/2\Z$.

In particular, if $p \geq 5$ is a prime of good reduction of~$E$ there is only one such value of~$d$.
\end{proposition}
