...
the methods from this paper together with those from~\cite{FKSym}. We have used our code to classify the symplectic types of congruences between curves in the LMFDB (see \cite{lmfdb}), namely all elliptic curves defined over~$\Q$ of conductor less than~$\numprint{400000}$. More precisely, Theorem~\ref{T:cong19} shows there are no mod~$p$ congruences in LMFDB for $p \geq 19$ and 
...
still useful.  In both these cases they may be found in
\cite{Rubin-Silverberg}.
...
equations, together with $X_E^-(7)$ and $X_E^{\pm}(11)$ were found by Fisher in~\cite{Fisher}.

Below we will study congruences modulo~$p$ systematically 
for~$p\ge7$,
...
p\le23$) while for the last two we restrict to $p=7$ which is the most interesting case, as remarked in the Introduction.
%% as there is a
...
have irreducible mod~$11$ representations.  Up to quadratic twist,
there are only $89$ distinct subsets.

For $p=13$ we find $150$ congruent subsets, all pairs.  Up to
quadratic twist there are just $19$ pairs.

For $p=17$ we find $8$ congruent subsets, all pairs.  These are all
quadratic twists of the pair $\{\lmfdbec{3675}{o}{1},
\lmfdbec{47775}{da}{1}\}$.
...
For $p=13$, the $150$ nontrivial subsets are all of size~$2$, and
all are irreducible.

For $p=17$, the $8$ nontrivial subsets are all of size~$2$, and
all are irreducible.
...
For $p\ge11$ there are no reducible pairs to consider.
